...
function icoActive($r){
	global $actionPage;
	$onChange = 'onChange="self.location=\'?mod='.LURL::dotPage($actionPage).'&action=changestatus&IndId='.$r->IndId."&start=".$_REQUEST["start"].'&EnableStatus=\'+this.value"';
...
function icoEdit($r){
	...
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($addPage)."&id=".$r->IndId."&start=".$_REQUEST["start"]."'",
		'ico edit',
		$label,
		$label
	));
...
function icoView($r){
	...
	global $viewPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($viewPage)."&id=".$r->IndId."&start=".$_REQUEST["start"]."'",
		'ico view noicon',
		$label,
		$label
	));
...
function icoDelete($r){
	...
	global $actionPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript: toDelete('?mod=".LURL::dotPage($actionPage)."&action=delete&id=".$r->IndId."&start=".$_REQUEST["start"]."')",
		'ico delete',
		$label,
		$label
	));
...
<div id="boxSearch" class="boxsearch" style="display:none;">
 <table  border="0" align="center" cellpadding="4" cellspacing="5" >
  	<tr>
      <td  align="left"><strong>แผนหลัก 5 ปี :</strong></td>
      <td align="left">
	  <select name="main_plan_id" id="main_plan_id">
	 	 <option value="">งบประมาณ 2555-2559</option>
		 <option value="">งบประมาณ 2560-2564</option>
	  </select></td>
      <td rowspan="2" align="left"><input id="search22" name="search22" type="button" value="  ค้นหา  " class="btnRed"   onclick="Search();" />
        <input type="button" name="button52" id="button52" class="btn" value=" ยกเลิก " onclick="JQ('#boxSearch').hide();JQ('#boxFilter').show();" /></td>
  	</tr>
    <tr>
      <td  align="left"><strong>คำค้น : </strong></td>
      <td align="left"><input name="tsearch" id="tsearch" type="text" class="input-search" size="30" value="<?php echo $_REQUEST['tsearch']?>" /></td>
    </tr>
  </table>
...
<table width="100%" border="1" class="tbl-list"  cellspacing="1">
<thead>
  <tr>
    <th class="no" style="width:30px">ลำดับ</th>
    <th style="width:100px;" >รหัสตัวชี้วัด</th>
    <th>ชื่อตัวชี้วัด</th>
    <th style="width:200px;">แผนหลัก 5 ปี</th>
...
?>
  <tr>
    ...
    <td valign="top" style="text-align:center;"><?php echo $IndCode; ?></td>
    <td valign="top" ><?php echo icoView($r);?></td>
    <td align="center" valign="top" >งบประมาณ 2560-2564</td>
