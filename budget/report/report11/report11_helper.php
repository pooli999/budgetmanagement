...
class sHelper
{
	...
	*/	
	...
	/* Function Name: getTotalCost4X */
	/* Description: เป็นฟังก์ชันสำหรับดึงยอดงบประมาณโครงการรายเดือน/ไตรมาส(เงินงบประมาณ) */
	function getTotalCost4X($CostExtId,$MonthNo){
		$where = array();
	...
	function getItemRequireExternal($CostItemCode=0){
		$where = array();
		if($_REQUEST["BgtYear"]){
			$where[] = "t4.BgtYear='".$_REQUEST["BgtYear"]."'";
		}
		...
		if($_REQUEST["PrjDetailId"]){
			$where[] = "t3.PrjDetailId='".$_REQUEST["PrjDetailId"]."'";
		}
		if($_REQUEST["PrjActId"]){
			$where[] = "t2.PrjActId='".$_REQUEST["PrjActId"]."'";
		}
		if($_REQUEST["ScreenLevel"]){
		...
		if($CostItemCode){
			$where[] = "t1.CostItemCode in('".$CostItemCode."')";
		}
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
