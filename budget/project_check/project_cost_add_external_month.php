...
function loadSCT(PrjActId){
	...
	JQ('#act').load('?mod=<?php echo lurl::dotPage("ajaxdata");?>&section=getCostDetailEx&format=raw&PrjActId=' + PrjActId+'&SourceExId='+SourceExId);
}
function loadCost(CostExtId){
	window.location.href = '?mod=<?php echo lurl::dotPage($AddCostExternalMonth);?>&CostItemId=<?php echo $_REQUEST["CostItemId"];?>&PrjDetailId=<?php echo $_REQUEST["PrjDetailId"];?>&CostItemCode=<?php echo $_REQUEST["CostItemCode"];?>&ParentCode=<?php echo $_REQUEST["ParentCode"];?>&OrganizeCode=<?php echo $_REQUEST["OrganizeCode"];?>&PrjId=<?php echo $_REQUEST["PrjId"];?>&SourceExId=<?php echo $_REQUEST["SourceExId"];?>&CostExtId=' + CostExtId + '&PrjActId=' + JQ('#PrjActId').val();
}
...
?>
 ...
 <tr>
  ...
  <tr>
    ...
    <td style="width:948px; text-align:left; font-weight:bold"><?php echo $get->getActName(ltxt::getVar('PrjDetailId'),ltxt::getVar('PrjActId'),'PrjActId');?></td>
  </tr>
   <tr>
...
?>
 ...
 <tr>
   ...
   <td style="width:948px; text-align:left; font-weight:bold"><div id="act"><?php echo $get->getCostDetailEx(ltxt::getVar('PrjActId'),$_REQUEST['SourceExId'],ltxt::getVar('CostExtId'),'CostExtId');?></div></td>
 </tr>

  <tr>
   <th style="width:200px; text-align:left">งบประมาณโครงการ</th>
   <?php $SumCost=$get->getCostEx($_REQUEST["CostExtId"]); ?>
   <td style="width:948px; text-align:left; font-weight:bold">
   <input type="text"name="SumCost" id="SumCost" style="width:50px; text-align:right;" value="<?php echo $SumCost; ?>" disabled="disabled"> 
   (<?php echo JThaiBaht::_($get->getCostEx($_REQUEST["CostExtId"])); ?>)</td>
 </tr>
</table>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="tbl-list">
  ...
  <tr>
    ...
    <td style="text-align:right; font-weight:bold;">รวมทั้งสิ้น<span style="text-align:right;">
      <input type="text" name="Total" id="Total" class="number" style="width:100px;" readonly="readonly" value="<?php echo ($Total)?$Total:0; ?>" onmouseover="showhint('ผลรวมงบประมาณทั้งสิ้น', this, event, '250px')"/>
    </span></td>
    </tr>
</table>
