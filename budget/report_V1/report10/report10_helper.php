...
class sHelper
{
	...
	*/	
	...
	function getProjectRecordSet($PItemCode){
		...
		$sql="SELECT distinct(t1.PrjDetailId),t2.*,t1.Purpose "
	...
	function getIndPlanRecordSet($PItemCode){
		$where = array();
	...
	function getIndPrjRecordSet($PrjDetailId){
		$where = array();
		$where[] = "t1.PrjDetailId='".$PrjDetailId."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
	...
	function getIndActRecordSet($PrjActId){
		$where = array();
	...
	function getTotalPrjMonth($PItemCode=0,$PrjDetailId=0,$PrjActId=0,$MonthNo=0){
		switch($_REQUEST["ExType"]){
			case "Internal":
					$BGInternal = $this->getTotalPrjMonthInternal($PItemCode,$PrjDetailId,$PrjActId,$MonthNo);
					return $BGInternal;
			break;
			case "External":
					$BGExternal = $this->getTotalPrjMonthExternal($PItemCode,$PrjDetailId,$PrjActId,$MonthNo);
					return $BGExternal;
			break;
			default:
					$BGInternal = $this->getTotalPrjMonthInternal($PItemCode,$PrjDetailId,$PrjActId,$MonthNo);
	...
	/* Parameter: */
			/* @XXXX	= - */
	/* Return Value : Field */
	function getTotalPrjMonthInternal($PItemCode=0,$PrjDetailId=0,$PrjActId=0,$MonthNo=0){
		$where = array();
		if($_REQUEST["BgtYear"]){
			$where[] = "t1.BgtYear='".$_REQUEST["BgtYear"]."'";
		}
	...
	/* Parameter: */
			/* @XXXX	= - */
	/* Return Value : Field */
	function getTotalPrjMonthExternal($PItemCode=0,$PrjDetailId=0,$PrjActId=0,$MonthNo=0){
		$where = array();
		if($_REQUEST["BgtYear"]){
			$where[] = "t1.BgtYear='".$_REQUEST["BgtYear"]."'";
		}
