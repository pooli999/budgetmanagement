 ...
 if($selectAct){
        foreach($selectAct as $r){
            foreach( $r as $k=>$v){ ${$k} = $v;} 
				...
				$SumCost=$get->getTotalPrjInternalX4($_REQUEST["BgtYear"],$OrganizeCode,0,0,0,$PrjActId,0,0);
				$SumCostEx=$get->getTotalPrjExternalX4($_REQUEST["BgtYear"],$OrganizeCode,0,0,0,$PrjActId,0,0,$SourceExId);
...
?>
	...
	?>
        <tr>
          <td colspan="7" style="text-align:center; height:30px" ><span style="color:#999;">-ไม่ระบุ-</span></td>
        </tr>
        <?php	
		}
    ?>
