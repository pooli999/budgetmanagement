...
class sHelper
{
	...
	*/	
	...
	function getIndList($selected=0,$tag_attribs='onchange="loadIndName(this.value)" style="width:99%"',$tag_name='IndCode',$lebel='ระบุ'){
		$where = array();
		$where[] ="EnableStatus='Y'";
		$where[] ="DeleteStatus<>'Y'";
		if(count($where)) {
		...
		$sql="SELECT IndCode as value , concat('(',IndCode,') ',IndName)as text "
	...
	function getIndName($IndCode=0){
		$where = array();
		$IndCode = ($_REQUEST["IndCode"])?$_REQUEST["IndCode"]:$IndCode;
		$where[] ="t1.IndCode='".$IndCode."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT concat('(',IndCode,') ',IndName) "
			 ."\n FROM tblbudget_indicator as t1 "
			 ."\n ".$where_r
			 ;
		//echo "<pre>$sql</pre>";
		$this->db->setQuery($sql);
	...
	function getLastRunningNo($LPlanCode){
