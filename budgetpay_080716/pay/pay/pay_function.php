...
class sFunction extends sHelper{
	...
	function Approve(){ 
		...
		if($pk = $this->db->arecSave('tblfinance_form_operate',$_REQUEST)){
			...
			$sql = "update tblintra_eform_formal_pay set DocStatusId=".$_REQUEST["DocStatusId"]." where DocCode='".$_REQUEST["DocCode"]."' ";
			$this->db->Execute($sql);		
	...
	function Cancel(){
		...
		if($pk = $this->db->arecSave('tblfinance_form_operate',$_REQUEST)){//insert ตารางสถานะการดำเนินการของเอกสาร
			...
			$sql = "update tblbudget_bg_book set CancelStatus='Y'  where DocCode='".$_REQUEST["DocCode"]."' ";
			$this->db->Execute($sql);//update CancelStatus ของตารางตัดจ่ายเงิน
			
			$sql2 = "update tblintra_eform_formal_pay set DocStatusId='8' where DocCode='".$_REQUEST["DocCode"]."' ";
			$this->db->Execute($sql2);	//update สถานะเอกสาร
			
			// update สถานะ เลขที่ สช.น. เป็น 8 (ยกเลิกเอกสาร) 
			$sql3 = "update tblfinance_form_doccode set DocStatusId=8 where DocCode='".$_REQUEST["DocCode"]."' ";
