...
class sFunction extends sHelper{
	...
	function Approve(){ 
		...
		if($pk = $this->db->arecSave('tblfinance_form_operate',$_REQUEST)){
			...
			$sql = "update tblintra_eform_advance_clear set DocStatusId=".$_REQUEST["DocStatusId"]." where DocCode='".$_REQUEST["DocCode"]."' ";
			$this->db->Execute($sql);		
	...
	function Cancel(){
		...
		if($pk = $this->db->arecSave('tblfinance_form_operate',$_REQUEST)){//insert ตารางสถานะการดำเนินการของเอกสาร
			...
			$sql2 = "update tblintra_eform_advance_clear set DocStatusId='8' where DocCode='".$_REQUEST["DocCode"]."' ";
