...
class sFunction extends sHelper{
	...
	function Approve(){ 
		...
		if($pk = $this->db->arecSave('tblfinance_form_operate',$_REQUEST)){
			...
			$sql = "update tblintra_eform_formal_mat_pay set DocStatusId=".$_REQUEST["DocStatusId"]." where DocCodePay='".$_REQUEST["DocCode"]."' ";
			$this->db->Execute($sql);		
	...
	function Cancel(){
		...
		if($pk = $this->db->arecSave('tblfinance_form_operate',$_REQUEST)){//insert ตารางสถานะการดำเนินการของเอกสาร
			...
			$sql2 = "update tblintra_eform_formal_mat_pay set DocStatusId='8' where DocCodePay='".$_REQUEST["DocCode"]."' ";
