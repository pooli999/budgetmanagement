...
<input type="button" name="button4" id="button4" value="บันทึกผล" class="btnRed" onclick="goPage('?mod=<?php echo lurl::dotPage($appPage);?>&id=<?php echo $_GET["id"]; ?>')"/>
    <?php } ?>
     <?php if(in_array($DocStatusId,array(12))){ ?>
<input type="button" name="button4" id="button4" value="ยกเลิกโอนเงิน" class="btnRed" onclick="goPage('?mod=<?php echo lurl::dotPage($cancelPage);?>&id=<?php echo $_GET["id"]; ?>')"/>
    <?php } ?>
