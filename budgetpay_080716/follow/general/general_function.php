...
class sFunction extends sHelper{
	...
	function Approve(){ ltxt::print_r($_REQUEST);
		$this->db->debug(2);
		if($pk = $this->db->arecSave('tblfinance_form_operate',$_REQUEST)){//จัดเก็บข้อมูลลงตารางผลการอนุมัติเอกสาร
			...
			foreach($ArrDoc as $val){
				if($val != 0){
					...
					$this->db->arecSave('tblfinance_form_operate_file',$Data);//จัดเก็บไฟล์แนบ
				}
			}	
			$sql = "update tblintra_eform_formal_general set DocStatusId=".$_REQUEST["DocStatusId"]." where DocCode='".$_REQUEST["DocCode"]."' ";
			$this->db->Execute($sql);//ปรับสถานะเอกสาร
/*			if($DocStatusId==7){ 
