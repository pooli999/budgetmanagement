...
function icoEdit($r){
	...
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($addPage)."&id=".$r->IncomeType."&IncomeTypeAcId=".$r->IncomeTypeAcId."&AcGroupId=".$r->AcGroupId."&AcSeriesId=".$r->AcSeriesId."&AcSeriesName=".$r->AcSeriesName."&start=".$_REQUEST["start"]."'",
		'ico edit',
		$label,
		$label
	));
...
<thead>
  <tr>
    ...
    <th align="left" >ประเภทรายรับ</th>
    <th align="left">ผูกบัญชีย่อย Cr.</th>
...
?>
  <tr>
    ...
    <td valign="top" ><?php // echo icoView($r);?><?php echo $IncomeType_Name;?></td>
    <td valign="top" >
<?php 
$AcList = $get->getAcDataList($IncomeType);//ltxt::print_r($AcList);
if($AcList["rows"]){
	$iv = 0;
