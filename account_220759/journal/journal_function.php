...
class sFunction extends sHelper{
	...
	*/	
	function Save(){
		if($pk = $this->db->arecSave('ac_journal',$_POST)){	
			if($_POST["journalId"]=='')
				LogFiles::SaveLog("ระบบบัญชี","เพิ่มข้อมูลสมุดบัญชีรายวัน",$pk,$_REQUEST["shortName"]." ".$_REQUEST["journalName"]);
			else
				LogFiles::SaveLog("ระบบบัญชี","แก้ไขข้อมูลสมุดบัญชีรายวัน",$pk,$_REQUEST["shortName"]." ".$_REQUEST["journalName"]);
