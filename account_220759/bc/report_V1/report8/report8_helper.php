...
class sHelper
{
	...
	*/	
	...
	function getTotalPrjMonth($OrganizeCode=0,$ScreenLevel=0){
		switch($_REQUEST["ExType"]){
			case "Internal":
					$BGInternal = $this->getTotalPrjMonthInternal($OrganizeCode,$ScreenLevel);
					return $BGInternal;
			break;
			case "External":
					$BGExternal = $this->getTotalPrjMonthExternal($OrganizeCode,$ScreenLevel);
					return $BGExternal;
			break;
			default:
					$BGInternal = $this->getTotalPrjMonthInternal($OrganizeCode,$ScreenLevel);
	...
	/* Parameter: */
			/* @XXXX	= - */
	/* Return Value : Field */
	function getTotalPrjMonthInternal($OrganizeCode=0,$ScreenLevel=0){
		$where = array();
		if($_REQUEST["BgtYear"]){
			$where[] = "t1.BgtYear='".$_REQUEST["BgtYear"]."'";
		}
	...
	/* Parameter: */
			/* @XXXX	= - */
	/* Return Value : Field */
	function getTotalPrjMonthExternal($OrganizeCode=0,$ScreenLevel=0){
		$where = array();
		if($_REQUEST["BgtYear"]){
			$where[] = "t1.BgtYear='".$_REQUEST["BgtYear"]."'";
		}
