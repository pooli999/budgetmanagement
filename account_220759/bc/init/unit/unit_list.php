...
function icoActive($r){
	global $actionPage;
	$onChange = 'onChange="self.location=\'?mod='.LURL::dotPage($actionPage).'&action=changestatus&UnitID='.$r->UnitID."&start=".$_REQUEST["start"].'&EnableStatus=\'+this.value"';
...
function icoEdit($r){
	...
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($addPage)."&id=".$r->UnitID."&start=".$_REQUEST["start"]."'",
		'ico edit',
		$label,
		$label
	));
...
function icoView($r){
	...
	global $viewPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($viewPage)."&id=".$r->UnitID."&start=".$_REQUEST["start"]."'",
		'ico view noicon',
		$label,
		$label
	));
...
function icoDelete($r){
	...
	global $actionPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript: toDelete('?mod=".LURL::dotPage($actionPage)."&action=delete&id=".$r->UnitID."&start=".$_REQUEST["start"]."')",
		'ico delete',
		$label,
		$label
	));
...
<thead>
  <tr>
    ...
    <th align="center" style="width:120px;">ตำแหน่งหน่วยนับ</th>
