...
<table width="100%" border="0" cellspacing="1" cellpadding="1" class="tbl-view">
   <tr>
        <th valign="top">ขั้นตอนการจัดทำงบประมาณ</th>
        <td ><?php echo  $get->getSCTypeName($_GET["SCTypeId"]);?></td>
   </tr>
   <tr>
			...
			if($data){
			   ...
			   foreach($data as $row){
				   foreach( $row as $k=>$v){${$k} = $v;}
					echo '<li class="ui-state-default" id="'.$ScreenId.'" >'.$ScreenName.'</li>';
					$i++;
				 } 
			}else{
				echo '<span style="color:#900">-- ไม่มีข้อมูล --</span>';
			}
            ?>
        </ul>  
    </div> 
   ...
   <tr>
      ...
      <tr>
        ...
        <td>
        <?php if($data){ ?>
        <input type="button" name="button4" id="button4" value="บันทึกลำดับ" class="btnRed" onclick="SaveOrder()"  />
        <?php } ?>
