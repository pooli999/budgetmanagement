...
// ดึงชื่อขั้นตอน
$CurSCType=$get->getSCTypeCurOrg($_REQUEST['BgtYear'],$_REQUEST["OrganizeCode"]); //ltxt::print_r($CurSCType);
$CurSCName=$get->getSCRName($_REQUEST['BgtYear'],$_REQUEST["OrganizeCode"]); //ltxt::print_r($CurSCName);
$NameByScreen=$get->getNameByScreen($_REQUEST['BgtYear'],$CurSCType[0]->ScreenLevel,$CurSCType[0]->SCTypeId,0); //ltxt::print_r($NameByScreen);
...
function icoEdit($r){
	...
	global $ListEdit;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($ListEdit)."&start=".$_REQUEST["start"]."&BgtYear=".$_REQUEST['BgtYear']."&OrganizeCode=".$_REQUEST['OrganizeCode']."&SCTypeId=".$r->SCTypeId."&ScreenLevel=".$r->ScreenLevel." '",
		'ico edit',
		$label,
		$text
	));
...
<span class="ico edit">  ตรวจสอบโครงการ</span>
</div>
...
?>
              <tr<?php if($flag){ ?> style="background-color:#FFFFCC; font-weight:bold;"<?php } ?>>
                ...
                <td style="width:30px;"><?php echo icoViewPrj($r_screen); ?></td>
                <td style="width:30px;"><?php if($flag){ echo icoEdit($r_screen); } ?></td>
              </tr>
<?php 
			$n++;
		}
 ...
 ?>
	<tr>
    	<td colspan="7" style="text-align:center; vertical-align:middle; color:#900">- - ไม่มีข้อมูล - -</td>
    </tr>
<?php } ?>
</table>