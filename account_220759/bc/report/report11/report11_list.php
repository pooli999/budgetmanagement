...
function printDocument(){
	window.location.href="?mod=<?php echo LURL::dotPage('report11_print')?>&format=raw<?php echo $get->getQueryString(); ?>";
}

function saveToExcel(){
	window.location.href="?mod=<?php echo LURL::dotPage('report11_excel')?>&format=raw<?php echo $get->getQueryString(); ?>";
}
...
function loadStep(BgtYear){
	...
	JQ.ajax({
		  type: "POST",
		  url: "?mod=<?php echo LURL::dotPage('report11_action');?>",		   
...
<input type="hidden" name="mod" id="mod" value="<?php echo LURL::dotPage('report11_list');?>" />
...
<?php
			$CR=0;
			$CRequireItemLevel1 = $get->getItemRequireExternal($CostItemCode);//ltxt::print_r($CRequireItemLevel1);
			foreach($CRequireItemLevel1 as $RCRequireItemLevel1){
				foreach($RCRequireItemLevel1 as $k=>$v){
					${$k} = $v;
				}
				unset($SumCostMonth1,$SumCostMonth2,$SumCostMonth3,$SumCostMonth4,$SumCostMonth5,$SumCostMonth6,$SumCostMonth7,$SumCostMonth8,$SumCostMonth9,$SumCostMonth10,$SumCostMonth11,$SumCostMonth12);
				if($SumCost){ //check รายการค่าใช้จ่าย level1
					//ไตรมาสที่ 1	
					$SumCostMonth10=$get->getTotalCost4X($CostExtId,10);
					$SumCostMonth11=$get->getTotalCost4X($CostExtId,11);
					$SumCostMonth12=$get->getTotalCost4X($CostExtId,12);
				
					//ไตรมาสที่ 2	
					$SumCostMonth1=$get->getTotalCost4X($CostExtId,1);
					$SumCostMonth2=$get->getTotalCost4X($CostExtId,2);
					$SumCostMonth3=$get->getTotalCost4X($CostExtId,3);
				
					//ไตรมาสที่ 3
					$SumCostMonth4=$get->getTotalCost4X($CostExtId,4);
					$SumCostMonth5=$get->getTotalCost4X($CostExtId,5);
					$SumCostMonth6=$get->getTotalCost4X($CostExtId,6);
				
					//ไตรมาสที่ 4	
					$SumCostMonth7=$get->getTotalCost4X($CostExtId,7);
					$SumCostMonth8=$get->getTotalCost4X($CostExtId,8);
					$SumCostMonth9=$get->getTotalCost4X($CostExtId,9);
				}
		   ...
		   echo ($Unit4)?(" x&nbsp;".number_format($Value4,2)."&nbsp;".$get->getUnitName($Unit4)):"";
		    echo '&nbsp;)</span>';
		   ?></td>
		   <td class="sum-total" title="งบเดือน/ไตรมาส"><?php echo ($SumCost > 0)?number_format($SumCost,2):"-"; ?></td>
		   <td class="sum-total" title="ต.ค"><?php echo ($SumCostMonth10 > 0)?number_format($SumCostMonth10,2):"-"; ?></td>
		   <td class="sum-total" title="พ.ย"><?php echo ($SumCostMonth11 > 0)?number_format($SumCostMonth11,2):"-"; ?></td>
		   <td class="sum-total" title="ธ.ค"><?php echo ($SumCostMonth12 > 0)?number_format($SumCostMonth12,2):"-"; ?></td>
		   <td class="sum-total" title="ม.ค"><?php echo ($SumCostMonth1 > 0)?number_format($SumCostMonth1,2):"-"; ?></td>
