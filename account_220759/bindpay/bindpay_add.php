...
function loadAC(){
	...
	//JQ.ajax({
		  //type: "GET",
		 // url: "?mod=<?php echo LURL::dotPage('bindpay_action');?>",		   
	...
	//});
	JQ.post(actionURL,{action:"action=loadac&AcGroupId="+AcGroupId+"&AcSeriesId="+AcSeriesId,node:JQ(this).val()},function(callback){
	...
	JQ(document).ready(function(){
		...
		<?php if($_GET["id"]!=""){?>
			JQ.ajax({
				...
				async: false,
				url: "?mod=<?php echo LURL::dotPage('bindpay_action');?>",		   
				data: "action=getacdata&CostItemId=<?php echo $_GET["id"];?>",
...
<div class="sysinfo">
  <div class="sysname"><? if ($_REQUEST["CostItemAcId"] == ""){?>เพิ่มรายการ<? }else{?>แก้ไขรายการ<? }?><?php echo $MenuName;?></div>
...
<input name="CostItemId" type="hidden"  id="CostItemId" value="<?php echo $_REQUEST['id'];?>" />
<input name="CostItemAcId" type="hidden"  id="CostItemAcId" value="<?php echo $_REQUEST['CostItemAcId'];?>" />
...
<input name="CostName" type="hidden"  id="CostName" value="<?php echo $CostName;?>" />
...
<td class="require">*</td>
<td colspan="3"><?php echo $CostName;?></td>
</tr>
<tr>
  <th>ชุดผังบัญชี</th>
  <td class="require">&nbsp;</td>
