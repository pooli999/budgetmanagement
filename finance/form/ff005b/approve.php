...
<div class="sysinfo">
  <div class="sysname"><span style="background-color:<?php echo $BGColor; ?>">&nbsp;FF005<?php //echo $FormCode; ?>&nbsp;</span>&nbsp;<?php echo $FormName; ?></div>
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" class="tbl-view">
	<tr>
		<th style="background-color: #6699FF; color:#FFFFFF" colspan="3">
			<?php

				if ($eform_type == "1"){
					echo "แบบฟอร์มขออนุมัติจัดประชุม";
				}else if($eform_type == "2"){
					echo "แบบฟอร์มขออนุมัติเดินทางปฏิบัติงาน";
				}else{
					echo "แบบฟอร์มขออนุมัติอื่นๆ";
				}
			?>
		</td>
	</tr>
  ...
  <tr>
	<th><?php
		if ($eform_type == "1"){
			echo "จำนวนผู้เข้าร่วมประชุม";
		}else{
			echo "จำนวนผู้เข้าร่วมปฏิบัติงาน";
		}
	?></th>
...
<!--END รายการค่าใช้จ่าย-->
 ...
 <tr>
    <th>ยอดงบขอยืมเงินทดรอง</th>
    <td><div style="width:120px; float:left; text-align:right; "><?php echo number_format($TotalBorrow,2); ?></div>&nbsp;<b>บาท</b></td>
  </tr>
 <tr>
    <th>ยอดงบขอวางบิลเบิกจ่าย</th>
    <td><div style="width:120px; float:left; text-align:right; "><?php echo number_format($TotalBilling,2); ?></div>&nbsp;<b>บาท</b></td>
  </tr>
...
function ValidateForm(f){
		if(JQ('#HasBorrow').val() == "Y"){
			if(JQ('#ReturnDate').val() == ""){
				jAlert('กรุณาระบุกำหนดวันคืนเงินยืมทดรอง',function(){
					JQ('#ReturnDate').focus();
				});
				return false;
			}
...
<input type="hidden" name="SumBGBorrow" id="SumBGBorrow" value="<?php echo $TotalBorrow; ?>" />
<input type="hidden" name="SumBGRemain" id="SumBGRemain" value="<?php echo $TotalBorrow; ?>" />
<input type="hidden" name="HasBorrow" id="HasBorrow" value="<?php echo $HasBorrow; ?>" />
...
<?php if($HasBorrow == "Y"){ ?>
  <tr>
   <th>กำหนดคืนเงินยืมทดรอง</th>
   <td>
<?php
		if($ReturnDate=="") $ReturnDate = date('Y-m-d');
	  	echo InputCalendar_text(array(
			'id'=> 'ReturnDate',
			'name' => 'ReturnDate',
			'value' => ""
