...
."\n tblfinance_bg_pay as t8 "
	...
	function getDataDocCodeRefer($DocCode){
		...
		$sql="SELECT t1.* "
			 ."\n FROM tblfinance_bg_chain as t1 "
			 ."\n ".$where_r
			 ;
		$this->db->setQuery($sql);//echo "<pre>".$sql."</pre>";
		$data = $this->db->loadObjectList();
	...
	/* START */
	/* Function Name: getSumSumCostBill */
	/* Description: เป็นฟังก์ชันสำหรับดึงผลรวมของยอดวางบิลเบิกจ่าย */
	function getSumSumCostBill($DocCodeRefer,$CostItemCode=0,$notDocCode=0){
		$where = array();
		$where[] = "t2.DocCodeRefer='".$DocCodeRefer."'";
		if($CostItemCode){
			$where[] = "t1.CostItemCode='".$CostItemCode."'";
	...
	/* START */
	/* Function Name: getChainItem */
	/* Description: เป็นฟังก์ชันสำหรับดึงรายการผูกพันงบประมาณ */
	function getChainItem($DocCodeRefer){
		$where = array();
		$where[] = "t1.DocCode='".$DocCodeRefer."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT t1.*,t2.CostName "
				."\n FROM "
				."\n tblfinance_bg_chain as t1 "
				."\n Inner Join tblbudget_init_cost_item AS t2 ON t2.CostItemCode = t1.CostItemCode "
				."\n ".$where_r
				;
		$this->db->setQuery($sql);//echo "<pre>".$sql."</pre>";
		$datas = $this->db->loadObjectList();//echo "<pre>".$datas."</pre>";
	...
	/* START */
	/* Function Name: getSumBGChainPay */
	/* Description: เป็นฟังก์ชันสำหรับดึงผลรวมของยอดวางบิลเบิกจ่าย */
	function getSumBGChainPay($DocCodeRefer,$notDocCode=0,$CostItemCode=0){
		$where = array();
		$where[] = "t2.DocCodeRefer='".$DocCodeRefer."'";
		if($notDocCode){
			$where[] = "t1.DocCode not in('".$notDocCode."')";
		...
		$sql="SELECT t1.SumBGPay "
				."\n FROM "
				."\n tblfinance_bg_pay as t1 "
				."\n Inner Join tblfinance_doccode AS t2 ON t2.DocCode = t1.DocCode "
				."\n ".$where_r
				;
		$this->db->setQuery($sql);//echo "<pre>".$sql."</pre>";
		$datas = $this->db->loadResult();//echo "<pre>".$datas."</pre>";
	...
	/* Description: เป็นฟังก์ชันสำหรับดึงผลรวมของยอดผูกพันงบ */
	function getSumBGChain($DocCodeRefer,$CostItemCode=0){
		$where = array();
		$where[] = "t1.DocCode='".$DocCodeRefer."'";
		if($CostItemCode){
			$where[] = "t1.CostItemCode='".$CostItemCode."'";
