...
function ValidateForm(f){
		...
		if(JQ('#AmountPerson').val() == ""){
			jAlert('กรุณากรอกช่องจำนวนผู้เข้าร่วมประชุม/ผู้ร่วมเดินทาง',function(){
				JQ('#AmountPerson').focus();
			});
			return false;
		}
		if(JQ('#CountryCode').val() == 0){
			jAlert('กรุณากรอกช่องเดินทางไปประเทศจุดหมาย',function(){
				JQ('#CountryCode').focus();
			});
			return false;
		}
...
<?php 
 ...
 ?>
  ...
  <tr>
    ...
    <td class="require">*</td>
    <td><?php echo $get->getMasterCountry($CountryCode);?></td>
  </tr>  
...
<div style="float:left; width:120px;">
	<?php 
		if($StartDate2=="") $StartDate2 = date('Y-m-d');
	  	echo InputCalendar_text(array(
			'id'=> 'StartDate2',
			'name' => 'StartDate2',
			'value' => $StartDate2
