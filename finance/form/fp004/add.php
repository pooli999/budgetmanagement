...
function ValidateForm(f){
		...
		if(getValueTextEditor('Detail') == ""){
			jAlert('กรุณากรองช่องมีความความประสงค์ที่จะต้องดำเนินการ',function(){
				JQ('#Detail').focus();
			});
			return false;
		}
		if(parseFloat(JQ('#TotalCost').val().replace(/,/g,'')) == 0){
			jAlert('กรุณากรอกจำนวนเงินที่ต้องการขออนุมัติ',function(){
				JQ('#TotalCost').focus();
			});
			return false;
		}
		if(parseFloat(JQ('#TotalCost').val().replace(/,/g,'')) > 5000){
			jAlert('คุณกรอกจำนวนเงินเกิน 5 พันบาท กรุณาตรวจสอบความถูกต้องของยอดเงิน',function(){
				JQ('#TotalCost').focus();
			});
			return false;
		}
...
<?php 
 ...
 ?>
  ...
  <tr>
    <th colspan="3">มีความความประสงค์ที่จะต้องดำเนินการ <span class="require">*</span></th>
  </tr>  
  <tr>
    <td  colspan="3">
	<?php
