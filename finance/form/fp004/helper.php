...
."\n tblfinance_bg_pay as t8 "
	...
	/* Description: เป็นฟังก์ชันสำหรับดึงชื่อตำแหน่ง */
	function getPositionList($selected=0,$tag_name='PositionId',$tag_attribs='style="width:95%"',$lebel='เลือก'){
		$where = array();
		$where[] = "EnableStatus='Y'";
	...
	/* Description: เป็นฟังก์ชันสำหรับดึงริษัท/ห้าง/ร้าน */
	function getDuty($selected=0,$tag_name='DutyId',$tag_attribs='style="width:95%"',$lebel='เลือก'){
		$where = array();
		$where[] = "EnableStatus='Y'";
		$where[] = "DeleteStatus='N'";		
		$where[] = "DutyId in (1,2,4)";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT DutyId as value , DutyName as text "
