...
class sHelper
{
	...
	function getFile($DocCode){
		...
		$sql = "select FileDocId from tblfinance_form_pay_file ".$where_r; 
		$this->db->setQuery($sql);//echo "<pre>".$sql."</pre>";
	...
	function getOrgRoundCode($selected=0,$tag_name='CKOrgRoundCode',$tag_attribs='onchange="loadOrgIdList(this.value)"',$lebel='เลือก'){
...
."\n tblfinance_bg_pay as t8 "
	...
	function getImpCostItemCode($DocCode){
		...
		$sql = "select distinct(CostItemCode)as CostItemCode from tblfinance_form_pay_cost ".$where_r; 
		$this->db->setQuery($sql);//echo "<pre>".$sql."</pre>";
		$datas = $this->db->loadObjectList();
		return $datas;
	}	
	...
	function getCostDetail($DocCode){
		...
		$sql="SELECT t1.* "
				."\n FROM "
				."\n tblfinance_form_pay_cost as t1 "
				."\n ".$where_r
				;
		$this->db->setQuery($sql);//echo "<pre>".$sql."</pre>";	
		$datas = $this->db->loadObjectList();//echo "<pre>".$datas."</pre>";	
	...
	function getAttachList($DocCode=0){
		...
		."\n FROM "
		."\n tblfinance_form_pay_attach as t1 "
	...
	function getAttachTrue($DocCode,$AttachCode){
		...
		$sql="select count(AttachCode) "
				."\n from tblfinance_form_pay_attach "
				."\n".$where_r
				;
		$this->db->setQuery($sql);//echo "<pre>$sql;</pre>";
		return $this->db->loadResult();
	...
	/* START */
	/* Function Name: getChildTypeList */
	/* Description: เป็นฟังก์ชันสำหรับดึงประเภทบุตร */
	function getChildTypeList(){
		$where = array();
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}		
		$sql="SELECT t1.TChildId, t1.TChildName "
		."\n FROM "
		."\n tblfinance_init_child as t1 "
		."\n ".$where_r
	...
	/* START */
	/* Function Name: getRelateData */
	/* Description: เป็นฟังก์ชันสำหรับดึงข้อมูลผู้รับสวัสดิการ */
	function getRelateData($PersonalCode,$RelateId=0){
		$where = array();
		...
		$sql="SELECT t1.FullName, t1.BirthDay, t1.RelateId "
		."\n FROM "
		."\n tblpersonal_welfare as t1 "
		."\n ".$where_r
		;
		$this->db->setQuery($sql);//echo "<pre>$sql;</pre>";
		$datas = $this->db->loadObjectList();
	...
	/* START */
	/* Function Name: getRelateDocCode */
	/* Description: เป็นฟังก์ชันสำหรับดึงข้อมูลผู้ขอเบิกค่ารักษาพยาบาล */
	function getRelateDocCode($DocCode,$RelateId){
		$where = array();
		$where[] = "DocCode='".$DocCode."'";
		$where[] = "RelateId='".$RelateId."'";
		if(count($where)) {
		...
		$sql="SELECT t1.* "
		."\n FROM "
		."\n tblfinance_form_pay_personal as t1 "
		."\n ".$where_r
		;
		$this->db->setQuery($sql);//echo "<pre>$sql;</pre>";
		$datas = $this->db->loadObjectList();
	...
	/* START */
	/* Function Name: getArrayRelateData */
	/* Description: เป็นฟังก์ชันสำหรับดึง Array ผู้รับสวัสดิการ */
	function getArrayRelateData($PersonalCode){
		$where = array();
		$where[] = "PersonalCode='".$PersonalCode."'";
		$where[] = "EnableStatus='Y'";
		$where[] = "DeleteStatus <> 'Y'";
		...
		$sql="SELECT t1.RelateId "
		."\n FROM "
		."\n tblpersonal_welfare as t1 "
		."\n ".$where_r
		;
		$this->db->setQuery($sql);//echo "<pre>$sql;</pre>";
		$datas = $this->db->loadResultArray();//ltxt::print_r($datas);
		$arr = array();
	...
	/* START */
	/* Function Name: checkPayPerson1 */
	/* Description: เป็นฟังก์ชันสำหรับดึงชื่อเอกสารแนบ */	
	function checkPayPerson($DocCode,$RelateId){
		$where = array();
		$where[] = "DocCode='".$DocCode."'";
		$where[] = "RelateId='".$RelateId."'";
		if($where){
		...
		$sql="select count(*) "
				."\n from tblfinance_form_pay_personal "
				."\n".$where_r
				;
		$this->db->setQuery($sql);//echo "<pre>$sql;</pre>";
		return $this->db->loadResult();
	...
	/* START */
	/* Function Name: getTrueTChildId */
	/* Description: เป็นฟังก์ชันสำหรับดึงชื่อเอกสารแนบ */	
	function getTrueTChildId($PPersonId,$TChildId){
		$where = array();
		$where[] = "PPersonId='".$PPersonId."'";
		$where[] = "TChildId='".$TChildId."'";
		if($where){
			$where_r = "\n where ".implode(" and ",$where);
		}
		$sql="select count(*) "
				."\n from tblfinance_form_pay_personal_child "
				."\n".$where_r
				;
		$this->db->setQuery($sql);//echo "<pre>$sql;</pre>";
		return $this->db->loadResult();
	...
	/* START */
	/* Function Name: getRelateDocCodeNoFullName */
	/* Description: เป็นฟังก์ชันสำหรับดึงข้อมูลผู้ขอเบิกค่ารักษาพยาบาล */
	function getRelateDocCodeNoFullName($DocCode,$RelateId){
		$where = array();
		$where[] = "DocCode='".$DocCode."'";
		$where[] = "RelateId='".$RelateId."'";
		if(count($where)) {
		...
		$sql="SELECT t1.*,t1.FullName as oldFullName "
		."\n FROM "
		."\n tblfinance_form_pay_personal as t1 "
		."\n ".$where_r
		;
	...
	function getOrgShortNameListApprove($CKOrgRoundCode=0,$selected=0,$tag_name='CKOrganizeCode',$tag_attribs='onchange="loadPersonList(this.value)" style="width:80%"',$lebel='เลือก'){
				$CKOrgRoundCode = ($CKOrgRoundCode)?$CKOrgRoundCode:$_REQUEST["CKOrgRoundCode"];
				if($selected == ''){
					$selected = $_SESSION['Session_OrgCode'];
				}
				jimport('packages.utility.utl_grouptree');
				$Tree = $this->getTree($CKOrgRoundCode);
	...
	function getPersonList($CKOrgRoundCode=0,$CKOrganizeCode=0,$selected=0,$tag_name='CKPersonalCode',$tag_attribs='onchange="loadPositionList(this.value);" style="width:200px"',$lebel='เลือก'){
		$where = array();	
		$CKOrgRoundCode = ($CKOrgRoundCode)?$CKOrgRoundCode:$_REQUEST["CKOrgRoundCode"];
		$where[] = " t3.OrgRoundCode='".$CKOrgRoundCode."'";
		$CKOrganizeCode = ($CKOrganizeCode)?$CKOrganizeCode:$_REQUEST["CKOrganizeCode"];
		$where[] = " t1.OrganizeCode='".$CKOrganizeCode."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
	...
	function getPositionList($CKPersonalCode=0,$selected=0,$tag_name='CKPositionId',$tag_attribs='style="width:200px"',$lebel='เลือก'){
		$CKPersonalCode = ($CKPersonalCode)?$CKPersonalCode:$_REQUEST["CKPersonalCode"];
		$where = array();
		if($CKPersonalCode){
			$selectedP =  $this->getPositionByPersonalCode($CKPersonalCode);
			if($selected == ''){
				$selected = $selectedP;
			}	
		}
	...
	function getCountPayNo($BgtYear,$PersonalCode){
		$where = array();
		$where[] = " BgtYear='".$BgtYear."'";
		$where[] = " PersonalCode='".$PersonalCode."'";
		$where[] = " DocStatusId not in(8,13)";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT max(PayNo) "
			 ."\n FROM tblfinance_form_pay "
			 ."\n ".$where_r
			 ;	
		//echo "<pre>".$sql."</pre>";		
	...
	function getSumPayWelfare($notDocCode){
		$where = array();
		$where[] = "DocCode not in('".$notDocCode."')";
		$where[] = "FormCode='FH004'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
