...
."\n tblfinance_bg_pay as t8 "
	...
	function getBGItemHold($DocCode){
		$where = array();
		$where[] = "t1.DocCode='".$DocCode."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
	...
	function getDocBGItemHold($DocCode){
		$where = array();
		$where[] = "t1.DocCode='".$DocCode."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		...
		$sql="SELECT t1.* "
			 ."\n FROM tblfinance_form_hold_cost as t1 "
			 ."\n ".$where_r
			 ;
		$this->db->setQuery($sql);//echo "<pre>$sql</pre>";	 
		$datas = $this->db->loadObjectList();
	...
	function getPublicMaxNo($FormCode,$BgtYear){
		$where = array();
		$where[] = "t1.FormCode='".$FormCode."'";
	...
	function genPublicDocCode($FormCode,$BgtYear){
		$nextAutoNo = $this->getPublicMaxNo($FormCode,$BgtYear) ;
		$RunNo = sprintf("%04s",$nextAutoNo);
		$Code = "คชจ.".$RunNo."/".$BgtYear;
		return $Code;
	}
	
	/* START */
	/* Function Name: getAllBGHold */
	/* Description: เป็นฟังก์ชันสำหรับดึงผลรวมของยอดงบหลักการของเอกสาร */	
	function getAllBGHold($DocCodeRefer,$CostItemCode=0){
		$where = array();
		$where[] = "t1.DocCode='".$DocCodeRefer."'";
		if($CostItemCode){
			$where[] = "t1.CostItemCode='".$CostItemCode."'";
		}
		if(count($where)) {
	...
	/* START */
	/* Function Name: getAllBGChain */
	/* Description: เป็นฟังก์ชันสำหรับดึงผลรวมของยอดงบผูกพันของเอกสาร */	
	function getAllBGChain($DocCodeRefer,$CostItemCode=0){
		$where = array();
		$where[] = "DocCodeRefer='".$DocCodeRefer."'";
		if($CostItemCode){
			$where[] = "CostItemCode='".$CostItemCode."'";
		}
		if(count($where)) {
		...
		$sql="SELECT sum(SumBGChain) "
				."\n FROM "
				."\n tblfinance_bg_chain "
				."\n ".$where_r
				;
		$this->db->setQuery($sql);//echo "aaaaa<pre>".$sql."</pre>";	
		$datas = $this->db->loadResult();//echo "bbbbbbbbbb<pre>".$datas."</pre>";	
		return $datas;
	}
	/* END */	
