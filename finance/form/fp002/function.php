...
class sFunction extends sHelper{
	...
	function Check(){ 
		...
		$sql = "Update tblintra_eform_formal_mat set DocStatusId='".$_POST["DocStatusId"]."' where DocCode = '".$_POST["DocCode"]."'";
		$this->db->Execute($sql);
	...
	function Approve(){ 
		...
		if($_POST["DocStatusId"] == 7){
			$chain["Title"] = $this->getTitle($_POST["DocCode"]);
			$itemHold = $this->getBGItemHold($_POST["DocCode"]);//ltxt::print_r($itemHold);
			foreach($itemHold as $itemHoldRow){
				foreach($itemHoldRow as $k=>$v){
					$chain[$k] = $v;
				}
				$chain["SumBGChain"] = $chain["SumBGHold"];
				$chain["DocCodeRefer"] = $chain["DocCode"];
				//ลดยอดหลักการของเอกสาร
				$TotalCost = $chain["SumBGChain"];
				$TotalChain = $chain["SumBGChain"];
				$TotalRemain = $TotalCost-$TotalChain;
