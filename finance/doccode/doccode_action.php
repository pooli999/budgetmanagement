	...
	switch( strtolower($action) ) {
		...
		case 'projectlist':
			...
			$OrganizeCode = ltxt::getVar( 'OrganizeCode','post' );
		 	$PItemId = ltxt::getVar( 'PItemId','post' );
			echo $tsk->getProjectList($BgtYear,$OrganizeCode,$PItemId);
			exit;
		break;			
		...
		case 'projectlistto':
			...
			$OrganizeCodeTo = ltxt::getVar( 'OrganizeCodeTo','post' );
		 	$PItemIdTo = ltxt::getVar( 'PItemIdTo','post' );
			echo $tsk->getProjectList($BgtYearTo,$OrganizeCodeTo,$PItemIdTo,0,'PrjIdTo','onchange="loadActTo(this.value)" style="width:80%"');
			exit;
		break;					
...
/*		case 'personallist':
		...
		case 'bgprj':
			...
			$PrjId = ltxt::getVar( 'PrjId','post' );
		 	$PItemId = ltxt::getVar( 'PItemId','post' );
			echo $tsk->getBGPrj($OrganizeCode, $PrjId, $PItemId);
			exit;
		break;	
		...
		case 'bgact':
			...
			echo $tsk->getBGAct($OrganizeCode, $PrjId, $PItemId,$PrjActCode);
			exit;
		break;	
		...
		case 'bgprjto':
			...
			$PrjIdTo = ltxt::getVar( 'PrjIdTo','post' );
		 	$PItemIdTo = ltxt::getVar( 'PItemIdTo','post' );
			echo $tsk->getBGPrjTo($OrganizeCodeTo, $PrjIdTo, $PItemIdTo);
			exit;
		break;	
		...
		case 'bgactto':
			...
			echo $tsk->getBGActTo($OrganizeCodeTo, $PrjIdTo, $PItemIdTo,$PrjActCodeTo);
			exit;
		break;	
		...
		case 'pitemlistto':
		 	$BgtYearTo = ltxt::getVar( 'BgtYearTo','post' );
			echo $tsk->getPlanItemList($BgtYearTo,9,0,'PItemIdTo','onchange="loadPrjTo(this.value)" style="width:80%"');
			exit;
		break;	
