...
class sHelper
{
	...
	*/	
	function getPaymentData(){
		...
		$where[] = "nh_in_partner_detail.PartnerId <>0 and nh_in_partner_detail.AddressGroupId = 1"; // ·Õè·Ó§Ò¹
		if($where){
			$where_r = "\n where ".implode(" and ",$where);
		}
		
		$sql="SELECT distinct  nh_in_partner.PartnerCode ,tblfinance_payment_comp.PaymentCompId,tblfinance_payment_comp.PaymentId,tblfinance_payment_comp.PartnerCode,tblfinance_payment_comp.CashValue,tblfinance_payment_comp.Tax,tblfinance_payment_comp.TaxW,AddressDetail,Soi,Road,DistrictCode,SubDistrictCode,ProvinceCode,PtnPrefixTH,PtnFname,PtnSname,TaxIdent ,tblfinance_payment_comp.ChequePayDate,tblfinance_payment_comp.ChequeOrCash"
