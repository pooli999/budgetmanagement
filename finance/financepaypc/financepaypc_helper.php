...
class sHelper
{
	...
	*/
	function getDataList2(&$list,$limit=20){ // ตารางจ่ายเงิน
		...
		$sql="select tblfinance_payment.*,$fn1,$fn2,$fn3,$fn4 from tblfinance_payment ".$where_r." and tblfinance_payment.PaymentId IN(select PaymentId from tblfinance_payment_list_detail where FormCode = 'FF009')";
		//echo $sql;
		$this->db->setQuery($sql);
		$this->db->limit = $limit;
	...
	*/
	...
	/*END*/
	function getPaymentDetailFinance($PaymentId){
     $sql = "select PaymentDetailFinance from tblfinance_payment where PaymentId = ".$PaymentId;
		 //echo $sql;
     $this->db->setQuery($sql);
     $PaymentDetailFinance = $this->db->loadResult();
