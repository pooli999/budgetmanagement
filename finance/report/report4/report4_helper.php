...
class sHelper
{
	...
	*/	
	...
	function getItemRecordSet(){
		...
		$sql="SELECT t1.*,t2.* "
			 ."\n FROM tblfinance_bg_borrow as t1 "
			 ."\n left join tblfinance_form_chain as t2 on t2.DocCode=t1.DocCode "
			 ."\n left Join tblbudget_project_activity AS t3 ON t3.PrjActCode = t1.PrjActCode "
			  ."\n ".$where_r
			  ."\n order by t1.BorrowId desc "
			 ;
		$this->db->setQuery($sql);//echo "<pre>$sql</pre>";
		$datas = $this->db->loadObjectList();
		return $datas;
	...
	function getClearItemRecordSet($BorrowCode){
		$where = array();
		$where[] = "t1.FormCode='FF007'";
		$where[] = "t1.BorrowCode='".$BorrowCode."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT t1.* "
			 ."\n FROM tblfinance_form_paybr as t1 "
			  ."\n ".$where_r
			  ."\n order by t1.ClearNo asc "
			 ;
		$this->db->setQuery($sql);//echo "<pre>$sql</pre>";
		$datas = $this->db->loadObjectList();
		return $datas;
