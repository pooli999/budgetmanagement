...
<table width="100%" border="0" class="tbl-list"  cellspacing="0" cellpadding="0">
  <tr style="vertical-align:top;">
    ...
    <th nowrap="nowrap">ชื่อกิจกรรม/เอกสาร</th>
    <th style="width:80px;">งบตามแผน<br />/งบขออนุมัติ</th>
    <th style="width:80px;">งบรับโอน</th>
    <th style="width:80px;">งบโอนออก</th>
    <th style="width:90px;">งบแผนรวมโอน</th>
    <th style="width:80px;">งบหลักการ</th>
    <th style="width:80px;">งบผูกพัน</th>
    <th style="width:80px;">งบเบิกจ่าย</th>
    <th style="width:80px;">งบคงเหลือ</th>
    <th style="width:80px;">งบเงินยืม</th>
    </tr>
<?php
$nAct = 1; 
...
foreach($recAct as $recActRow){ 
	...
	$BGPlan = $get->getSumBGPlan($_REQUEST["PrjDetailId"],$PrjActId);
	$BGTferIn = $get->getSumBGTferIn($_REQUEST["PrjDetailId"],$PrjActId);
	$BGTferOut = $get->getSumBGTferOut($_REQUEST["PrjDetailId"],$PrjActId);
	$BGPlanTfer = $BGPlan+$BGTferIn-$BGTferOut;
	$BGHold = $get->getSumBGHold($_REQUEST["PrjDetailId"],$PrjActId);
	$BGChain = $get->getSumBGChain($_REQUEST["PrjDetailId"],$PrjActId);
	$BGPay = $get->getSumBGPay($_REQUEST["PrjDetailId"],$PrjActId);
	$BGRemain =  $BGPlanTfer-$BGHold-$BGChain-$BGPay;
	$BGBorrow = $get->getSumBGBorrow($_REQUEST["PrjDetailId"],$PrjActId);
...
?>
  <tr style="vertical-align:top; background-color:#EEE; ">
    ...
    <td class="sum-total"><?php echo ($BGBorrow > 0)?number_format($BGBorrow,2):"-"; ?></td>
    </tr>
...
foreach($recItem as $recItemRow){ 
	...
	/*$fChain = array('FC001','FF001','FF002','FF003','FH001','FH002','FP001','FP002');
	if(in_array($FormCode,$fChain)){
		$BGHold		= $get->getSumBGHold($DocCode);
		$BGChain	= $get->getSumBGChain($DocCode);
		$BGBorrow	= $get->getSumBGBorrow($DocCode);
		$BGPay		= $get->getSumBGPay($DocCode);
		//$BGRemain	= $get->getSumBGHold($DocCode);
	}*/
	unset($DocBGHold,$DocBGChain,$DocBGPay,$DocBGBorrow);
	$DocBGHold = $get->getSumBGHold($_REQUEST["PrjDetailId"],$PrjActId,$DocCode);
	$DocBGChain = $get->getSumBGChain($_REQUEST["PrjDetailId"],$PrjActId,$DocCode);
	$DocBGPay = $get->getSumBGPay($_REQUEST["PrjDetailId"],$PrjActId,$DocCode);
	$DocBGBorrow = $get->getSumBGBorrow($_REQUEST["PrjDetailId"],$PrjActId,$DocCode);
?>
  <tr style="vertical-align:top;">
    <td style="text-align:center;"><?php echo ShowDate($DocDate); ?></td>
    <td style="text-align:center;"><?php echo $DocCode; ?></td>
    <td><?php echo $Title; ?></td>
    <td class="sum-total"><?php echo ($Budget > 0)?number_format($Budget,2):"-"; ?></td>
    <td class="sum-total">&nbsp;</td>
    <td class="sum-total">&nbsp;</td>
    <td class="sum-total">&nbsp;</td>
    <td class="sum-total"><?php echo ($DocBGHold > 0)?number_format($DocBGHold,2):""; ?></td>
    <td class="sum-total"><?php echo ($DocBGChain > 0)?number_format($DocBGChain,2):""; ?></td>
    <td class="sum-total"><?php echo ($DocBGPay > 0)?number_format($DocBGPay,2):""; ?></td>
    <td class="sum-total">&nbsp;</td>
    <td class="sum-total"><?php echo ($DocBGBorrow > 0)?number_format($DocBGBorrow,2):""; ?></td>
    </tr>
...
<?php
$sub = 1; 
$subItem = $get->getSubItemRecordSet($DocCode);//ltxt::print_r($recItem);
if(count($subItem)){
...
<!--Sub รายการเอกสาร level2-->    
<?php
$sub2 = 1; 
$subItem2 = $get->getSubItemRecordSet($DocCode);//ltxt::print_r($recItem);
if(count($subItem2)){
	foreach($subItem2 as $subItemRow2){ 
		foreach($subItemRow2 as $bb2=>$aa2){
			${$bb2} = $aa2;
		}
		unset($DocBGHold,$DocBGChain,$DocBGPay,$DocBGBorrow);
		$DocBGHold = $get->getSumBGHold($_REQUEST["PrjDetailId"],$PrjActId,$DocCode);
...
$BGRemain =  $BGPlanTfer-$BGHold-$BGChain-$BGPay;
$BGBorrow = $get->getSumBGBorrow($_REQUEST["PrjDetailId"],$_REQUEST["PrjActId"]);
?>
