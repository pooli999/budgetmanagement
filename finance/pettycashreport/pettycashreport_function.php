...
class sFunction extends sHelper{
	...
	*/	
	function Save(){
		...
		$CompensateId = $_REQUEST["CompensateId"];
		
		$sql = "Update tblfinance_doccode set CompensateId= 0 where CompensateId = ".$CompensateId; // อัพเดท รหัส เบิกคืนเงินสดย่อย ที่ efoem กรณี ลบ
		$this->db->Execute($sql);
		$sql = "Update tblbudget_finance_pettycash_compensate set DeleteStatus='Y' where CompensateId = ".$CompensateId;
		$this->db->Execute($sql);
		LogFiles::SaveLog("ระบบนการเงิน","รายงานการเบิกทดแทนเงินสดย่อย",$CompensateId,$_REQUEST["CompensateId"]);
