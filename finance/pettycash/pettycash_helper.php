...
class sHelper
{
	...
	*/	
	...
	//	$fn1 = "(select BankName from tblbudget_finance_bank where tblbudget_finance_bank.BankId = tblbudget_finance_bookbank.BankId) as BankName";
		...
		$sql="select PettyCashId,MaxPettyCash,AlertPettyCash,CreateDate from tblbudget_finance_pettycash ".$where_r."  order by Ordering ASC";
	...
	*/	
	function getDetail($PettyCashId){
		$where 	  = array();
		if($PettyCashId){
		...
		$sql = "select * from tblbudget_finance_pettycash ".$where_r;
		$this->db->setQuery($sql);
	...
	*/	
	function getOrderList(){
		...
		$sql="select * from tblbudget_finance_pettycash ".$where_r."  order by Ordering ASC";
		$this->db->setQuery($sql);
		$this->db->limit = $limit;
