...
function icoEdit($r){
	...
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($addPage)."&id=".$r->DocCode."&FormCode=".$_REQUEST["FormCode"]."&start=".$_REQUEST["start"]."'",
		'ico edit',
		$label,
		$label
	));
...
function icoDelete($r){
	...
	global $actionPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript: toDelete('?mod=".LURL::dotPage($actionPage)."&action=delete&id=".$r->DocId."&DocCode=".$r->DocCode."&FormCode=".$_REQUEST["FormCode"]."&start=".$_REQUEST["start"]."')",
		'ico delete',
		$label,
		$label
	));
...
function icoView($r){
	...
	global $viewPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($viewPage)."&id=".$r->DocCode."&FormCode=".$_REQUEST["FormCode"]."&start=".$_REQUEST["start"]."'",
		'ico view noicon',
		$label,
		$label
	));
...
function icoCancel($r){
	...
	global $cancelPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($cancelPage)."&id=".$r->DocCode."&FormCode=".$_REQUEST["FormCode"]."&start=".$_REQUEST["start"]."'",
		'ico cancel',
		$label,
		$label
	));
...
<div class="sysdetail">แสดงรายการเอกสาร</div>
</div>
...
-->
    ...
    <td style="text-align:right;">
      	<input type="button" value="  ย้อนกลับ  " class="btn" onclick="goPage('?mod=budgetpay.pay.main_pay&BgtYear=<?php echo $_REQUEST["BgtYear"]; ?>');" />
	</td>
  </table>
...
?> 
  <tr>
    ...
    <td style="text-align:right; vertical-align:text-top"><?php echo number_format($get->getSumBudget($DocCode,0,0,0),2); ?></td>
