...
$this->DOC->setPathWays(array(
	...
	array(
		'text' => 'แบบฟอร์มขออนุมัติเบิกจ่ายค่าจัดประชุม/เดินทางไปปฏิบัติงาน',
	)
));
...
function icoEdit($r){
	...
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($appPage)."&id=".$r->DocCodePay."&FormCode=".$_REQUEST["FormCode"]."&start=".$_REQUEST["start"]."'",
		'ico edit',
		$label,
		$label
	));
...
function icoView($r){
	...
	global $viewPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($viewPage)."&id=".$r->DocCodePay."&FormCode=".$_REQUEST["FormCode"]."&start=".$_REQUEST["start"]."'",
		'ico view noicon',
		$label,
		$label
	));
...
function icoCancel($r){
	...
	global $cancelPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($cancelPage)."&id=".$r->DocCodePay."&FormCode=".$_REQUEST["FormCode"]."&start=".$_REQUEST["start"]."'",
		'ico cancel',
		$desc,
		$label
	));
...
/* <![CDATA[ */
	...
	function loadSCT(BgtYear){
			...
			JQ.ajax({
			   type: "POST",
			   url: "?mod=<?php echo LURL::dotPage('meeting_action');?>",		   
...
<?php
	...
	if($list["rows"]){
          foreach($list["rows"] as $r ) {
				...
				//งบเบิกจ่ายที่มีการขออนุมัติไว้
				$TotalPayCost = $get->getSumBudget($DocCodePay,0,0,0);
				
				//งบเบิกจ่ายที่ไม่ได้คาดการณ์ไว้ก่อน				
				$TotalPayOther = $get->getSumCostOther($DocCodePay,0,0,0,0,0);
				
				//งบเบิกจ่ายรวม
				$TotalPay = $TotalPayCost + $TotalPayOther;				
?> 
  <tr>
    ...
    <td style="text-align:left; vertical-align:text-top"><?php echo $DocCodePay;?></td>
    <td style="text-align:left; vertical-align:text-top"><?php echo icoView($r);?></td>
    <td style="text-align:right; vertical-align:text-top"><?php echo number_format($TotalPay,2); ?></td>
