...
class sHelper
{
	...
	*/	
	...
	function getTotalPrjMonth($CostTypeId=0,$LevelId=0,$ParentCode=0,$HasChild="",$CostItemCode=0,$ScreenLevel=0){
		switch($_REQUEST["ExType"]){
			case "Internal":
					$BGInternal = $this->getTotalPrjMonthInternal($CostTypeId,$LevelId,$ParentCode,$HasChild,$CostItemCode,$ScreenLevel);
					return $BGInternal;
			break;
			case "External":
					$BGExternal = $this->getTotalPrjMonthExternal($CostTypeId,$LevelId,$ParentCode,$HasChild,$CostItemCode,$ScreenLevel);
					return $BGExternal;
			break;
			default:
					$BGInternal = $this->getTotalPrjMonthInternal($CostTypeId,$LevelId,$ParentCode,$HasChild,$CostItemCode,$ScreenLevel);
	...
	/* Parameter: */
			/* @XXXX	= - */
	/* Return Value : Field */
	function getTotalPrjMonthInternal($CostTypeId=0,$LevelId=0,$ParentCode=0,$HasChild="",$CostItemCode=0,$ScreenLevel=0){
		$where = array();
		if($_REQUEST["BgtYear"]){
	...
	/* Parameter: */
			/* @XXXX	= - */
	/* Return Value : Field */
	function getTotalPrjMonthExternal($CostTypeId=0,$LevelId=0,$ParentCode=0,$HasChild="",$CostItemCode=0,$ScreenLevel=0){
		$where = array();
		if($_REQUEST["BgtYear"]){
			$where[] = "t1.BgtYear='".$_REQUEST["BgtYear"]."'";
		}
	...
	function getScreenYear($BgtYear){
		...
		/*$sql="SELECT distinct(t2.ScreenLevel) , t1.ScreenName "
			 ."\n FROM tblbudget_init_screen_item as t1 "
			 ."\n inner join tblbudget_project_detail as t2 on t2.ScreenLevel=t1.ScreenLevel"
			 ."\n inner join tblbudget_project as t3 on t3.PrjId=t2.PrjId"
			 ."\n ".$where_r
			 ."\n order by t1.ScreenLevel ASC "
			 ;*/
