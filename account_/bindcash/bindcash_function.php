...
class sFunction extends sHelper{
	...
	*/	
	function Save(){
		if($pk = $this->db->arecSave('ac_cash',$_POST)){	
			if($_POST["CashAcId"]=='')
				LogFiles::SaveLog("ระบบบัญชี","เพิ่มข้อมูลผูกบัญชีเงินสด",$pk,$_REQUEST["CashName"]);
			else
				LogFiles::SaveLog("ระบบบัญชี","แก้ไขข้อมูลผูกบัญชีเงินสด",$pk,$_REQUEST["CashName"]);
