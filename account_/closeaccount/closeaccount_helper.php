...
class sHelper
{
	...
	*/	
	function getDataList(&$list,$limit=20){
		...
		$fn6 = '(select AcActionId from ac_action where ac_action.PaymentId = tblfinance_payment.PaymentId) as AcActionId';
		$fn7 = '(select PostStatus from ac_action where ac_action.PaymentId = tblfinance_payment.PaymentId) as PostStatus';
		$fn8 = '(select CloseStatus from ac_action where ac_action.PaymentId = tblfinance_payment.PaymentId) as CloseStatus';
		...
		$sql="select tblfinance_payment.*,$fn1,$fn2,$fn3,$fn4,$fn5,$fn6,$fn7,$fn8 from tblfinance_payment inner join tblfinance_payment_comp on tblfinance_payment.PaymentId = tblfinance_payment_comp.PaymentId  ".$where_r."  order by tblfinance_payment.PaymentId DESC";
		//echo $sql;
		//$sql="select tblfinance_payment.*,$fn1,$fn2,$fn3,$fn4 from tblfinance_payment inner join tblfinance_payment_comp as tc on tblfinance_payment.PaymentId = tc.PaymentId ".$where_r."  order by PaymentId DESC";
...
,tblfinance_payment_list.DocCode,tblbudget_project.PrjCode from tblfinance_payment_list inner join tblfinance_doccode on tblfinance_payment_list.DocCode=tblfinance_doccode.DocCode inner join tblbudget_project on tblfinance_doccode.PrjId = tblbudget_project.PrjId ".$where_r." ";
	...
	function getaccount(){
		...
		if ($AcActionId == ""){ // undefined load ผูกบัญชี
				...
				$sql = "select ChequeOrCash,CashValue from tblfinance_payment_comp where PaymentId = ".$PaymentId;// หาก่อนว่า เงินสดหรือธนาคาร
				$this->db->setQuery($sql);
				$this->db->limit = $limit;
				$list2 = $this->db->loadDataSet();
				if($list2["rows"]){
				...
				if($list2["rows"]){
					 foreach($list2["rows"] as $r1 ) {
						...
						if ($ChequeOrCash == 1){	//ดึงบัญชีธนาคาร  
							$data_send[$line]["CRValue"]=$r1->PaymentValue; // จำนวนเงินค่าใช้จ่าย
						}else{//ดึงบัญชีเงินสด
							$data_send[$line]["CRValue"]=$CashValue; // จำนวนเงินค่าใช้จ่าย
						}
