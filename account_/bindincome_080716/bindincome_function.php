...
class sFunction extends sHelper{
	...
	*/	
	function Save(){
		if($pk = $this->db->arecSave('tblbudget_finance_income_type_ac',$_POST)){	
			if($_POST["IncomeTypeAcId"]=='')
				LogFiles::SaveLog("ระบบบัญชี","เพิ่มข้อมูลผูกบัญชีรายรับ",$pk,$_REQUEST["IncomeType_Name"]);
			else
				LogFiles::SaveLog("ระบบบัญชี","แก้ไขข้อมูลผูกบัญชีรายรับ",$pk,$_REQUEST["IncomeType_Name"]);
