...
<?php
$massage = $get->getCheckInStatus($DocCode,$PrjActCode,$SourceExId);
if($massage){
	echo '<div style="padding:5px; background-color:#EEE; color:#990000; text-align:center;">'.$massage.'</div>';
}
?>
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tbl-button">
  <tr>
    <td>
<?php if(in_array($DocStatusId,array(1,4,5,6,11))){ ?>    
<?php 
if(!$massage){
?>
<a href="javascript:editDocument('<?php echo $FormCode; ?>','<?php echo $DocCode; ?>');" class="ico edit">ปรับปรุงเอกสาร</a>
&nbsp;
<?php } ?>
     <a href="javascript:toDelete('?mod=front.form.<?php echo strtolower($FormCode); ?>.action&action=delete&FormCode=<?php echo $FormCode; ?>&DocCode=<?php echo $DocCode; ?>')" class="ico deletedata" >ลบเอกสาร</a>&nbsp;
     <a href="javascript:toCancel('?mod=front.form.<?php echo strtolower($FormCode); ?>.action&action=cancel&FormCode=<?php echo $FormCode; ?>&DocCode=<?php echo $DocCode; ?>')" class="ico cancel">ยกเลิกเอกสาร</a>&nbsp;
    <a href="javascript:toSendEform('?mod=front.form.<?php echo strtolower($FormCode); ?>.action&action=sent&FormCode=<?php echo $FormCode; ?>&DocCode=<?php echo $DocCode; ?>')" class="ico sent">ส่งไปตรวจสอบและอนุมัติ</a>&nbsp;
<?php } ?>     
     <a href="javascript:printDocument();" class="icon-printer">พิมพ์เอกสาร</a>&nbsp;
    <a href="javascript:saveToWord();" class="icon-word">ส่งออกเป็น Word</a>&nbsp;
    <a href="javascript:saveAttachFile('<?php echo $FormCode; ?>','<?php echo $DocCode; ?>');" class="icon-attach">แนบไฟล์เพิ่มเติม</a>
    <?php if(in_array($DocStatusId,array(1,4,5,6,11))){ ?>    
     &nbsp;<a href="javascript:closeDocument('<?php echo $FormCode; ?>','<?php echo $DocCode; ?>');" class="icon-close">ปิดหลักการ</a>
<?php } ?>     
