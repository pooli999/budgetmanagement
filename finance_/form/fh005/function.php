...
function Check(){ 
		...
		$sql = "Update tblfinance_form_pay set DocStatusId='".$_POST["DocStatusId"]."' where DocCode = '".$_POST["DocCode"]."'";
		$this->db->Execute($sql);
	...
	function Approve(){ 
		...
		if($_POST["DocStatusId"]==5){
			...
			$sql = "Update tblfinance_bg_pay set ApproveStatus='Y' where DocCode = '".$_POST["DocCode"]."'";
			$this->db->Execute($sql);
		}
