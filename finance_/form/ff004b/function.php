...
class sFunction extends sHelper{
	...
	function Approve(){ 
		...
		if($_POST["DocStatusId"]==5){
			...
			if($_POST["HasBorrow"]=="Y"){
				//save ตารางเงินยืมทดรอง
				$_POST["RunNo"] = $this->getMaxRunNo($_POST["BgtYear"])+1;
				$nextRunNo = sprintf("%03s",$_POST["RunNo"]);
				$_POST["BorrowCode"] = $nextRunNo."/".$_POST["BgtYear"];
				$this->db->arecSave('tblfinance_bg_borrow',$_POST);
			}	
