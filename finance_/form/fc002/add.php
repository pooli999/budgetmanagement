...
function ValidateForm(f){
		...
		if(JQ('#Title').val() == ""){
			jAlert('กรุณากรอกช่องชื่อข้อตกลง',function(){
				JQ('#Title').focus();
			});
			return false;
		}
		...
		if(JQ('#NHPerson').val() == ""){
			jAlert('กรุณากรอกช่องชื่อผู้รับข้อตกลง',function(){
				JQ('#NHPerson').focus();
			});
			return false;
		}
		if(JQ('#NHPosition').val() == ""){
			jAlert('กรุณากรอกช่องตำแหน่งผู้รับข้อตกลง',function(){
				JQ('#NHPosition').focus();
			});
			return false;
		}
...
<?php
$dataRefer = $get->getHoldFormDetail($_REQUEST["DocCodeRefer"]);//ltxt::print_r($dataRefer);
...
<thead>
  		<tr>
    		...
    		<td  style="width:100px; text-align:right;">งบหลักการ</td>
...
$sumTotalChain 			= 0; 
$sumTotalRemain 		= 0;
$gCostItemCode = $get->getImpCostItemCodeHold($DocCode);//ltxt::print_r($gCostItemCode);
...
if($gCostItemCode[0]->CostItemCode){
	foreach($gCostItemCode as $gCostItemCoderow){
		...
		$sumSumCost 			= $get->getSumSumCostHold($_REQUEST["DocCodeRefer"],$CostItemCode);
		$sumSumChain 			= $get->getSumSumChain($_REQUEST["DocCodeRefer"],$CostItemCode,$_REQUEST["DocCode"]);
		$sumSumRemain			= $sumSumCost-$sumSumChain;
		
		$sumTotalCost 			= $sumTotalCost+$sumSumCost;
		$sumTotalChain 			= $sumTotalChain+$sumSumChain; 
		$sumTotalRemain 		= $sumTotalRemain+$sumSumRemain;
?>
<tr style="font-weight:bold;">
  <td style="text-align:center;"><?php echo ($m+1); ?></td>
	...
	<?php echo number_format($sumSumChain,2);?>
    <input type="hidden" name="ReferSumChain[<?php echo $CostItemCode; ?>]" id="ReferSumChain[<?php echo $CostItemCode; ?>]" value="<?php echo $sumSumChain; ?>" />
    </td>
	<td  style="text-align:right;" >
	<?php echo number_format($sumSumRemain,2);?>
    <input type="hidden" name="ReferSumRemain[<?php echo $CostItemCode; ?>]" id="ReferSumRemain[<?php echo $CostItemCode; ?>]" value="<?php echo $sumSumRemain; ?>" />
    </td>
...
?>
          <tr style="font-weight:bold;">
            ...
            <td  style="text-align:right;" ><?php echo number_format($sumTotalCost,2);?></td>
            <td  style="text-align:right;" ><?php echo number_format($sumTotalChain,2);?></td>
            <td  style="text-align:right;" ><?php echo number_format($sumTotalRemain,2);?></td>
            <td >บาท</td>
  		</tr>
<?php }else{ ?>     
	<tr>
    	<td colspan="6" style="text-align:center; color:#999;">-ไม่พบรายการ-</td>
    </tr> 
<?php } ?>        
	</table>    
...
<?php 
 ...
 ?>
  ...
  <tr style="vertical-align:top;">
    ...
    <td class="require">&nbsp;</td>
    <td><?php 
		if($AnalystDate=="") $AnalystDate = date('Y-m-d');
	  	echo InputCalendar_text(array(
			'id'=> 'AnalystDate',
			'name' => 'AnalystDate',
			'value' => $AnalystDate
  ...
  <tr style="vertical-align:top;">
    ...
    <td class="require">&nbsp;</td>
    <td><input type="text" name="NHOrg" id="NHOrg" value="<?php echo $NHOrg; ?>" style="width:400px;" /></td>
  </tr>
    <tr style="vertical-align:top;">
    <th>ชื่อผู้รับข้อตกลง</th>
    <td class="require">*</td>
    <td><input type="text" name="NHPerson" id="NHPerson" value="<?php echo $NHPerson; ?>" style="width:400px;" /></td>
  </tr>
  <tr style="vertical-align:top;">
    <th>ตำแหน่งผู้รับข้อตกลง</th>
    <td class="require">*</td>
    <td><input type="text" name="NHPosition" id="NHPosition" value="<?php echo $NHPosition; ?>" style="width:400px;" /></td>
  </tr>


    <tr>
    <th colspan="3">รายการประมาณการค่าใช้จ่าย <span class="hint" style="font-weight:normal;">(ต้องไม่เกินยอดงบหลักการคงเหลือของแต่ละรายการค่าใช้จ่ายที่ได้ขออนุมัติหลักการไว้)</span></th>
  </tr>  
