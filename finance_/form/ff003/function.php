...
class sFunction extends sHelper{
	...
	function Approve(){ 
		...
		if($_POST["DocStatusId"] == 7){
			$chain["FormCode"] = $_POST["FormCode"];
			$chain["BgtYear"] = $_POST["BgtYear"];
			$chain["PItemCode"] = $_POST["PItemCode"];
			$chain["PrjDetailId"] = $_POST["PrjDetailId"];
			$chain["PrjId"] = $_POST["PrjId"];
			$chain["PrjActCode"] = $_POST["PrjActCode"];
			$chain["SourceExId"] = $_POST["SourceExId"];
			$itemDocHold = $this->getDocBGItemHold($_POST["DocCode"]);//ltxt::print_r($itemDocHold);
			foreach($itemDocHold as $itemDocHoldRow){
				foreach($itemDocHoldRow as $kk=>$vv){
					$chain[$kk] = $vv;
				}
				$chain["Title"] = $chain["DetailCost"];
				$chain["SumBGChain"] = $chain["SumCost"];
				$chain["SumBGFix"] = $chain["SumCost"];
				$chain["DocCodeRefer"] = $chain["DocCode"];
				$chain["ApproveStatus"] = "Y";
				$chain["DocCode"] = $this->genPublicDocCode($_POST["FormCode"],$_POST["BgtYear"]);
				$this->db->arecSave('tblfinance_bg_chain',$chain);
				$this->db->arecSave('tblfinance_bg_fix',$chain);
				//ลดยอดงบหลักการ
				$allBGHold = $this->getAllBGHold($_POST["DocCode"],$chain["CostItemCode"]);
				$allBGChain = $this->getAllBGChain($_POST["DocCode"],$chain["CostItemCode"]);
				$SumBGHold = $allBGHold-$allBGChain;
				$sql = "Update tblfinance_bg_hold set SumBGHold='".$SumBGHold."' where DocCode = '".$_POST["DocCode"]."' and CostItemCode='".$chain["CostItemCode"]."'  ";
				$this->db->Execute($sql);
			}//ltxt::print_r($chain);
			
			//ลดยอดหลักการของเอกสาร
			$allBGHold = $this->getAllBGHold($_POST["DocCode"]);
			$TotalChain = $this->getAllBGChain($_POST["DocCode"]);
			$TotalRemain =  $allBGHold-$TotalChain;
			$sql = "Update tblfinance_form_hold set TotalChain='".$TotalChain."',TotalRemain='".$TotalRemain."' where DocCode = '".$_POST["DocCode"]."' ";
			$this->db->Execute($sql);
			//END ลดยอดหลักการของเอกสาร
			
		}//end if DocStatusId=7
		//END ผูกพันงบประมาณ-------------------------------------------------------------------
