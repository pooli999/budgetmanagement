...
class sHelper
{
	...
	*/	
	...
	function getSumBGPay($MonthNo=0,$PItemCode=0,$PrjDetailId=0,$PrjActId=0){
		$where = array();
		$PItemCode = ($_REQUEST["PItemCode"])?$_REQUEST["PItemCode"]:$PItemCode;
		$PrjDetailId = ($_REQUEST["PrjDetailId"])?$_REQUEST["PrjDetailId"]:$PrjDetailId;
		$PrjActId = ($_REQUEST["PrjActId"])?$_REQUEST["PrjActId"]:$PrjActId;
		if($MonthNo){
			$where[] = "MONTH(dc.BGPayDate)='".$MonthNo."'";
		}
		if($PItemCode){
			$where[] = "t1.PItemCode='".$PItemCode."'";
		}
		...
		$sql="SELECT sum(t8.SumBGPay)as total "
				...
				."\n left Join tblbudget_project_detail AS t2 ON t2.PrjDetailId = t3.PrjDetailId "
				."\n left Join tblbudget_project AS t1 ON t1.PrjId = t2.PrjId "
				."\n left Join tblfinance_doccode AS dc ON dc.DocCode = t8.DocCode "
				."\n ".$where_r
				;
		$this->db->setQuery($sql);//echo "<pre>".$sql."</pre>";	
		$datas = $this->db->loadResult();
