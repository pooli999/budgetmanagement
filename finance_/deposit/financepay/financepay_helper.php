...
class sHelper
{
	...
	*/	
	function getDataList(&$list,$limit=20){
		...
		$where[] ="CodeId > 25"; // ยังไม่จ่ายเงิน
	...
	*/	
	function getBookbankNumber($tag_name,$tag_attribs,$selected,$lebel,$CtrBankId){//$tag_name:ชื่อ element,$tag_attribs:attrib,$selected:ค่าที่ต้องการให้แสดง,$lebel: title ของ element
		$where = array();
	...
	*/	
	function getPaymentData(){
		...
		$where[] = "nh_in_partner_detail.PartnerId <>0 and nh_in_partner_detail.AddressGroupId = 1"; // ที่ทำงาน
		if($where){
			$where_r = "\n where ".implode(" and ",$where);
		}
