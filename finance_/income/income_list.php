...
function icoActive($r){
	global $actionPage;
	$onChange = 'onChange="self.location=\'?mod='.LURL::dotPage($actionPage).'&action=changestatus&IncomeId='.$r->IncomeId."&start=".$_REQUEST["start"].'&EnableStatus=\'+this.value"';
...
function icoEdit($r){
	...
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($addPage)."&id=".$r->IncomeId."&start=".$_REQUEST["start"]."'",
		'ico edit',
		$label,
		$label
	));
...
function icoView($r){
	...
	global $viewPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($viewPage)."&id=".$r->IncomeId."&start=".$_REQUEST["start"]."'",
		'ico view noicon',
		$label,
		$label
	));
...
function icoDelete($r){
	...
	global $actionPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript: toDelete('?mod=".LURL::dotPage($actionPage)."&action=delete&id=".$r->IncomeId."&start=".$_REQUEST["start"]."')",
		'ico delete',
		$label,
		$label
	));
...
<thead>
  <tr>
    ...
    <th align="left" ><span style="width:200px;">ประเภทรายรับ</span></th>
    <th align="left" >ได้รับเงินจาก</th>
    <th align="left" ><span style="width:150px;">ประเภทการรับเงิน</span></th>
    <th align="center" style="width:150px;"><span style="width:150px;">วันทีทำรายการ</span></th>
    <th align="center" style="width:150px;"><span style="width:150px;">จำนวนเงิน (บาท) </span></th>
...
?>
  <tr>
    ...
    <td valign="top" ><? echo $r->IncomeType_Name;?></td>
    <td valign="top" ><?
	echo $r->PayName;
	?></td>
    <td valign="top" ><?php
	...
	echo ShowDateLong($r->CreateDate);
	?></td>
    <td align="right" valign="top" ><?php echo number_format($r->IncomeValue,2);?></td>
	...
	<td style="width:60px;" valign="top"  nowrap="nowrap" ><a class="ico print" title="แก้ไข" href="/report/receipt_voucher.php?IncomeId=<?php echo $r->IncomeId ;?>"  target="_blank">ใบสำคัญรับ</a></td>
  </tr>
