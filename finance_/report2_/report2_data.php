...
$modData = $ArrMod[count($ArrMod)-1];
	...
	switch( strtolower($modData) ) {
		...
		case $KeyPage."_add":
			...
			{
				$id = ltxt::getVar('id');
				foreach( $datas = $get->getDurableRecieveDetail($id) as $k => $v ){${$k} = $v;}
			}
		break;
		...
		case $KeyPage."_view":
			if( ltxt::getVar( 'id','get' ) ) 
			{
				...
				$dataitem = array();
				$get->getMatdurItems( $dataitem );
			}	
		break;
		
		case $KeyPage."_view_item":
			if( ltxt::getVar( 'itemid','get' ) ) 
			{				
				$itemid = ltxt::getVar('itemid');
				foreach( $datas = $get->getDurableRecieveItemDetail($itemid) as $k => $v ){${$k} = $v;}
			}	
		break;
		
		case $KeyPage."_edit_item":
			if( ltxt::getVar( 'itemid','get' ) ) 
			{
				$itemid= ltxt::getVar('itemid');
				foreach( $get->getDurableRecieveItemDetail($itemid) as $k => $v ){${$k} = $v;}
			}	
		break;
