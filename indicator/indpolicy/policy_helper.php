...
/*		if($_REQUEST["tsearch"]){
	...
	*/		
	...
	function getQTIndMonthResult($PIndCode,$MonthNo){
		$where 	  = array();
		$where[] ="PIndCode='".$PIndCode."'";
		$where[] ="MonthNo='".$MonthNo."'";
		if($where){
	...
	function getQLIndMonthResult($PIndCode,$MonthNo){
		$where 	  = array();
		$where[] ="PIndCode='".$PIndCode."'";
		$where[] ="MonthNo='".$MonthNo."'";
		if($where){
	...
	function getMTargetScore($PIndCode,$MonthNo){
		$where = array();
		$where[] = "PIndCode = '".$PIndCode."'";
		$where[] = "MonthNo = '".$MonthNo."'";
		if(count($where)) {
	...
	function getCalQTScore($PIndId,$scoreResult){
		$indicator = $this->getIndDetail($PIndId);//ltxt::print_r($indicator);
	...
	function getCalQLScore($PIndId,$scoreResult){
		$indicator = $this->getIndDetail($PIndId);
		foreach($indicator as $in){ foreach( $in as $a=>$q){ ${$a} = $q;} }
		switch($scoreResult){
