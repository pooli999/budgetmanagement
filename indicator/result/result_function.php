...
class sFunction extends sHelper{
	...
	function Save(){
		ltxt::print_r($_POST);
		$this->db->debug(2);
		$sql = "DELETE FROM tblbudget_project_activity_result where PrjActCode = '".$_POST["PrjActCode"]."' and MonthNo=".$_POST["MonthNo"]." ";
		$this->db->Execute($sql);
		if($pk = $this->db->arecSave('tblbudget_project_activity_result',$_POST)){
			...
			$sql = "DELETE FROM tblbudget_project_activity_result_file where ResultId = ".$_POST["ResultId"]."  ";
			$this->db->Execute($sql);
			// Add Files
			foreach($ArrDoc as $val){
				if($val != 0){
					...
					$this->db->arecSave('tblbudget_project_activity_result_file',$Data);
				}
			}
		...
		//== Log File ===
			...
			LTXT::_( '?mod='.LURL::dotPage('result_projectmonth').'&PrjDetailId='.$_POST["PrjDetailId"], 'redirect' );
	...
	function SavePrj(){
		ltxt::print_r($_POST);
		$this->db->debug(2);
		$sql = "DELETE FROM tblbudget_project_result where PrjCode = '".$_POST["PrjCode"]."' and MonthNo=".$_POST["MonthNo"]." ";
		$this->db->Execute($sql);
		if($_POST['PrjResultId'] = $this->db->arecSave('tblbudget_project_result',$_POST)){
			$ArrDoc = explode(",",$_POST["MultiDocId"]);
			$sql = "DELETE FROM tblbudget_project_result_file where PrjResultId = ".$_POST["PrjResultId"]."  ";
			$this->db->Execute($sql);
			// Add Files
			foreach($ArrDoc as $val){
				if($val != 0){
					...
					$Data["CreateDate"] = date("Y-m-d H:i:s");
					$Data["UpdateBy"] = $_POST["UpdateBy"];				
					$this->db->arecSave('tblbudget_project_result_file',$Data);
				}
			}
			LTXT::_( '?mod='.LURL::dotPage('result_projectmonth').'&PrjDetailId='.$_POST["PrjDetailId"], 'redirect' );
	...
	function Delete(){		
		$sql = "delete from tblbudget_project_activity_result_file where ResultId = ".$_REQUEST["ResultId"]." ";
		$this->db->Execute($sql);
	...
	function SaveIndMonth(){
		...
		LTXT::_( '?mod='.LURL::dotPage("result_planmonth").'&PItemCode='.$_POST["PItemCode"], 'redirect' );
	}
