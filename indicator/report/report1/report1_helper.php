...
class sHelper
{
	...
	*/	
	...
	function getIndDetail($LindId){
		$where 	  = array();
		$_REQUEST["LindCode"] = ($_REQUEST["LindCode"])?$_REQUEST["LindCode"]:$LindCode;
		if($_REQUEST["LindCode"]){
			$where[] ="t1.LindCode='".$_REQUEST["LindCode"]."'";
		}
		if($_REQUEST["PLongCode"]){
			$where[] ="t2.PLongCode='".$_REQUEST["PLongCode"]."'";
		}
		if($_REQUEST["LPlanCode"]){
			$where[] ="t2.LPlanCode='".$_REQUEST["LPlanCode"]."'";
		}
		if($where){
			$where_r = "\n where ".implode(" and ",$where);
		}
		$sql="SELECT t1.*,t2.LPlanName "
				."\n FROM "
	...
	function getYearMainPlan($selected=0,$tag_name='PLongCode',$tag_attribs='onchange="loadPlan(this.value)"',$lebel='ทั้งหมด'){
		$where 	  = array();
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function getPlanItemList($selected=0,$tag_name='LPlanCode',$tag_attribs='style="width:99%;" onchange="loadInd(this.value)"',$label='ทั้งหมด'){
		$where = array();
		$selected = ($selected)?$selected:$_REQUEST["LPlanCode"];
		$where[] = "t1.PLongCode='".$_REQUEST["PLongCode"]."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT distinct(t1.LPlanCode)as value , t1.LPlanName as text "
	...
	function getPLongName($PLongCode){
	...
	function getPItemName($PLongCode,$LPlanCode){
		...
		$sql="select LPlanName "
				."\n from tblbudget_longterm_plan "
				."\n".$where_r
				;
		$this->db->setQuery($sql);
		$datas = $this->db->loadResult();
	...
	function getIndList($PrjDetailId=0,$selected=0,$tag_attribs='style="width:99%"',$tag_name='LindCode',$lebel='ทั้งหมด'){
		$where = array();
		$selected = ($selected)?$selected:$_REQUEST["LindCode"];
		$where[] ="t1.LPlanCode='".$_REQUEST["LPlanCode"]."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT t1.LindCode as value , concat('(',t1.LindCode,') ',t1.LindName)as text "
			 ."\n FROM tblbudget_longterm_plan_indicator as t1 "
			 ."\n ".$where_r
			 ."\n order by t1.LindCode "
			 ;
		//echo "<pre>$sql;</pre>";
		$this->db->setQuery($sql);
		$title[] = clssHTML::makeOption(0,$lebel);
