...
class sHelper
{
	...
	function getPrjLinkFiles($PrjResultId){
		$sql = "SELECT * FROM tblbudget_project_report3_file where PrjResultId='$PrjResultId' ";
		$this->db->setQuery($sql);
	...
	function getLinkFiles($ResultId){
		$sql = "SELECT * FROM tblbudget_project_activity_report3_file where ResultId='$ResultId' ";
		$this->db->setQuery($sql);
		$detail = $this->db->loadObjectList(); 
		$Arr = array();
		foreach($detail as $val){
...
/*		if($_REQUEST["tsearch"]){
	...
	function getIndDetail($IndicatorCode=0){
		$where 	  = array();
		$_REQUEST["IndicatorCode"] = ($_REQUEST["IndicatorCode"])?$_REQUEST["IndicatorCode"]:$IndicatorCode;
		if($_REQUEST["IndicatorCode"]){
			$where[] ="t1.IndicatorCode='".$_REQUEST["IndicatorCode"]."'";
		}
		if($_REQUEST["BgtYear"]){
			$where[] ="t3.BgtYear='".$_REQUEST["BgtYear"]."'";
		}
		if($_REQUEST["PItemCode"]){
			$where[] ="t3.PItemCode='".$_REQUEST["PItemCode"]."'";
		}
		if($_REQUEST["PrjDetailId"]){
			$where[] ="t1.PrjDetailId='".$_REQUEST["PrjDetailId"]."'";
		}
	...
	function getDetailPrj($PrjDetailId=0){
		$where = array();
		if($PrjDetailId){
			$where[] = "t1.PrjDetailId = '".$PrjDetailId."'";
		}
	...
	function getPlanIndDetail($PIndId=0){
		$where 	  = array();
		//$where[] ="t1.PIndId='".$PIndId."'";
		if($where){
			$where_r = "\n where ".implode(" and ",$where);
	...
	function getPItemDetail($PItemId=0){
		$where 	  = array();
		if($PItemId){
	...
	function getPlanItemList($selected=0,$tag_name='PItemCode',$tag_attribs='style="width:99%;" onchange="loadPrj(this.value)"',$label='ทั้งหมด'){
		$where = array();
		$selected = ($selected)?$selected:$_REQUEST["PItemCode"];
	...
	function getProjectList($selected=0,$tag_name='PrjDetailId',$tag_attribs='style="width:99%;" onchange="loadPrjInd(this.value)"',$label='ทั้งหมด'){
		$where = array();
		$where[] = "t1.ActiveStatus='Y'";
		$where[] = "t4.BgtYear='".$_REQUEST["BgtYear"]."'";
		$selected = ($selected)?$selected:$_REQUEST["PrjDetailId"];
		if($_REQUEST["PItemCode"]){
	...
	function getIndPrjList($PrjDetailId=0,$selected=0,$tag_attribs='style="width:99%"',$tag_name='IndicatorCode',$lebel='ทั้งหมด'){
		$where = array();
		$selected = ($selected)?$selected:$_REQUEST["IndicatorCode"];
		$where[] ="t1.PrjDetailId=".$_REQUEST["PrjDetailId"]."";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT t1.IndicatorCode as value , concat('(',t1.IndicatorCode,') ',t1.IndicatorName)as text "
			 ."\n FROM tblbudget_project_indicator as t1 "
			 ."\n ".$where_r
			 ."\n order by t1.IndicatorCode "
			 ;
	...
	function getIndPrjName($IndicatorCode=0){
		$where = array();
		$IndicatorCode = ($_REQUEST["IndicatorCode"])?$_REQUEST["IndicatorCode"]:$IndicatorCode;
		$where[] ="t1.IndicatorCode='".$IndicatorCode."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT concat('(',IndicatorCode,') ',IndicatorName) "
			 ."\n FROM tblbudget_project_indicator as t1 "
			 ."\n ".$where_r
			 ;
		//echo "<pre>$sql</pre>";
