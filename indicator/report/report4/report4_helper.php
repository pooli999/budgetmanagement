...
class sHelper
{
	...
	function getPrjResultDocName($PrjResultId){
		$where = array();
		$where[] = "t1.PrjResultId='".$PrjResultId."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql = "select t2.DocName "
				."\n from tblbudget_project_result_file as t1 "
				."\n left join tblintra_edoc_center as t2 on t2.DocId=t1.DocId "
				."\n ".$where_r
				;
		$this->db->setQuery($sql);
		$datas = $this->db->loadResultArray();
		$datas = implode(", ",$datas);
		$datas = ($datas)?$datas:'-';
		return $datas;
	}
	...
	function getResultDocName($ResultId){
		$where = array();
		...
		$sql = "select t2.DocName "
				."\n from tblbudget_project_activity_result_file as t1 "
				."\n left join tblintra_edoc_center as t2 on t2.DocId=t1.DocId "
				."\n ".$where_r
				;
		$this->db->setQuery($sql);
...
/*		if($_REQUEST["tsearch"]){
	...
	function getProjectView($PItemCode){
		$where = array();
	...
	function getPlanDetail($PItemCode=0){
		$where 	  = array();
		if($PItemCode){
			$where[] = "PItemCode='".$PItemCode."'";
		}
	...
	function getProjectListSearch($selected=0,$tag_name='PrjDetailId',$tag_attribs='style="width:99%;"',$label='ทั้งหมด'){
		$where = array();
		$where[] = "t1.ActiveStatus='Y'";
