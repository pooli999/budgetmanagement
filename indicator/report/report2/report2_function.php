...
class sFunction extends sHelper{
	...
	function Save(){
		...
		if($pk = $this->db->arecSave('tblbudget_project_activity_result',$_POST)){
			...
			$sql = "DELETE FROM tblbudget_project_activity_report2_file where ResultId = ".$_POST["ResultId"]."  ";
			$this->db->Execute($sql);
			// Add Files
			foreach($ArrDoc as $val){
				if($val != 0){
					...
					$this->db->arecSave('tblbudget_project_activity_report2_file',$Data);
				}
			}
		...
		//== Log File ===
			...
			LTXT::_( '?mod='.LURL::dotPage('report2_projectmonth').'&PrjDetailId='.$_POST["PrjDetailId"], 'redirect' );
	...
	function SavePrj(){
		...
		if($_POST['PrjResultId'] = $this->db->arecSave('tblbudget_project_result',$_POST)){
			$ArrDoc = explode(",",$_POST["MultiDocId"]);
			$sql = "DELETE FROM tblbudget_project_report2_file where PrjResultId = ".$_POST["PrjResultId"]."  ";
			$this->db->Execute($sql);
			// Add Files
			foreach($ArrDoc as $val){
				if($val != 0){
					...
					$Data["CreateDate"] = date("Y-m-d H:i:s");
					$Data["UpdateBy"] = $_POST["UpdateBy"];				
					$this->db->arecSave('tblbudget_project_report2_file',$Data);
				}
			}
			LTXT::_( '?mod='.LURL::dotPage('report2_projectmonth').'&PrjDetailId='.$_POST["PrjDetailId"], 'redirect' );
	...
	function Delete(){		
		$sql = "delete from tblbudget_project_activity_report2_file where ResultId = ".$_REQUEST["ResultId"]." ";
		$this->db->Execute($sql);
	...
	function SaveIndMonth(){
		...
		LTXT::_( '?mod='.LURL::dotPage("report2_planmonth").'&PItemCode='.$_POST["PItemCode"], 'redirect' );
	}
