...
class sHelper
{
	...
	function getPrjLinkFiles($PrjResultId){
		$sql = "SELECT * FROM tblbudget_project_report2_file where PrjResultId='$PrjResultId' ";
		$this->db->setQuery($sql);
	...
	function getLinkFiles($ResultId){
		$sql = "SELECT * FROM tblbudget_project_activity_report2_file where ResultId='$ResultId' ";
		$this->db->setQuery($sql);
		$detail = $this->db->loadObjectList(); 
		$Arr = array();
		foreach($detail as $val){
...
/*		if($_REQUEST["tsearch"]){
	...
	function getPlanIndDetail($PIndCode=0){
		$where 	  = array();
		$_REQUEST["PIndCode"] = ($_REQUEST["PIndCode"])?$_REQUEST["PIndCode"]:$PIndCode;
		if($_REQUEST["PIndCode"]){
			$where[] ="t1.PIndCode='".$_REQUEST["PIndCode"]."'";
		}
		if($_REQUEST["BgtYear"]){
			$where[] ="t2.BgtYear='".$_REQUEST["BgtYear"]."'";
		}
		if($_REQUEST["PItemCode"]){
			$where[] ="t2.PItemCode='".$_REQUEST["PItemCode"]."'";
		}
		if($where){
			$where_r = "\n where ".implode(" and ",$where);
		}
		$sql="SELECT t1.* "
				."\n FROM "
				."\n tblbudget_init_plan_item_indicator as t1 "
				."\n left join tblbudget_init_plan_item as t2 on t2.PItemCode=t1.PItemCode "
				."\n ".$where_r
				;
		$this->db->setQuery($sql);
		$data = $this->db->loadObjectList(); //ltxt::print_r($data);
	...
	function getPItemDetail($PItemCode=0){
		$where 	  = array();
		if($PItemCode){
			$where[] ="t1.PItemCode='".$PItemCode."'";
		}
	...
	function getPlanItemList($selected=0,$tag_name='PItemCode',$tag_attribs='style="width:99%;" onchange="loadInd(this.value)"',$label='ทั้งหมด'){
		$where = array();
	...
	function getIndList($PrjDetailId=0,$selected=0,$tag_attribs='style="width:99%"',$tag_name='PIndCode',$lebel='ทั้งหมด'){
		$where = array();
		$selected = ($selected)?$selected:$_REQUEST["PIndCode"];
		$where[] ="t1.PItemCode='".$_REQUEST["PItemCode"]."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT t1.PIndCode as value , concat('(',t1.PIndCode,') ',t1.PIndName)as text "
			 ."\n FROM tblbudget_init_plan_item_indicator as t1 "
			 ."\n ".$where_r
			 ."\n order by t1.PIndCode "
			 ;
		//echo "<pre>$sql;</pre>";
		$this->db->setQuery($sql);
		$title[] = clssHTML::makeOption(0,$lebel);
	...
	function getIndName($PIndCode=0){
		$where = array();
		$PIndCode = ($_REQUEST["PIndCode"])?$_REQUEST["PIndCode"]:$PIndCode;
		$where[] ="t1.PIndCode='".$PIndCode."'";
		if(count($where)) {
			$where_r = "WHERE ". implode( " AND ", $where );
		}
		$sql="SELECT concat('(',PIndCode,') ',PIndName) "
			 ."\n FROM tblbudget_init_plan_item_indicator as t1 "
			 ."\n ".$where_r
			 ;
		//echo "<pre>$sql</pre>";
