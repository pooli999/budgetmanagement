	...
	*/	
	function getDataList(&$list,$limit=20){
		if($_REQUEST["AcSeriesId"]!="0"){
			$where0="tblbudget_finance_bookbank.DeleteStatus='N'";
			$where1 = "tblbudget_finance_bookbank_ac.AcSeriesId =".$_REQUEST["AcSeriesId"];

			$sql="select *,";
			$sql = $sql."(select CONCAT(AcChartCode,' | ',ThaiName) from tblbudget_finance_bookbank_ac inner join ac_chart on tblbudget_finance_bookbank_ac.AcChartId=ac_chart.AcChartId where tblbudget_finance_bookbank.BookbankId=tblbudget_finance_bookbank_ac.BookbankId and ".$where1.") as acName,";
			$sql = $sql."(select BookbankAcId from tblbudget_finance_bookbank_ac where tblbudget_finance_bookbank.BookbankId=tblbudget_finance_bookbank_ac.BookbankId and ".$where1.") as BookbankAcId,";
			$sql = $sql."(select AcGroupId from tblbudget_finance_bookbank_ac where tblbudget_finance_bookbank.BookbankId=tblbudget_finance_bookbank_ac.BookbankId and ".$where1.") as AcGroupId,";
			$sql = $sql."'".$_REQUEST["AcSeriesId"]."' as AcSeriesId,'".$_REQUEST["AcSeriesName"]."' as AcSeriesName ";
			$sql = $sql."from (tblbudget_finance_bookbank inner join tblbudget_finance_bank on tblbudget_finance_bookbank.BankId=tblbudget_finance_bank.BankId) inner join tblbudget_finance_bookbank_type on tblbudget_finance_bookbank.BookbankTypeId=tblbudget_finance_bookbank_type.BookbankTypeId where ".$where0."  order by tblbudget_finance_bookbank.BookbankId ASC";
			//echo $sql;
	...
	Parameter		: 
		@ 	BookbankId	= ID (PK) ของตาราง tblbudget_finance_bookbank
	Return Value 	: Array 1 รายการ (loadSingleObject) 
	*/	
	function getDetail($BookbankId,$BookbankAcId){
		$where 	  = array();
		if($BookbankId){
			$where[] ="tblbudget_finance_bookbank.BookbankId=".$BookbankId;
		}
		if($BookbankAcId){
			$where[] ="tblbudget_finance_bookbank_ac.BookbankAcId=".$BookbankAcId;
		}
		if($where){
			$where_r = "\n where ".implode(" and ",$where);
		}
		
		$sql = "select * from ((tblbudget_finance_bookbank inner join tblbudget_finance_bank on tblbudget_finance_bookbank.BankId=tblbudget_finance_bank.BankId) inner join tblbudget_finance_bookbank_type on tblbudget_finance_bookbank.BookbankTypeId=tblbudget_finance_bookbank_type.BookbankTypeId) left join tblbudget_finance_bookbank_ac on tblbudget_finance_bookbank.BookbankId =tblbudget_finance_bookbank_ac.BookbankId ".$where_r;
		//echo $sql;
		$this->db->setQuery($sql);
