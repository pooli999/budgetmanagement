...
function icoActive($r){
	global $actionPage;
	$onChange = 'onChange="self.location=\'?mod='.LURL::dotPage($actionPage).'&action=changestatus&SourceExId='.$r->SourceExId."&start=".$_REQUEST["start"].'&EnableStatus=\'+this.value"';
...
function icoEdit($r){
	...
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($addPage)."&id=".$r->SourceExId."&start=".$_REQUEST["start"]."'",
		'ico edit',
		$label,
		$label
	));
...
function icoView($r){
	$label = $r->SourceExName;
	global $viewPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($viewPage)."&id=".$r->SourceExId."&start=".$_REQUEST["start"]."'",
		'ico view noicon',
		$label,
		$label
	));
...
function icoDelete($r){
	...
	global $actionPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript: toDelete('?mod=".LURL::dotPage($actionPage)."&action=delete&id=".$r->SourceExId."&start=".$_REQUEST["start"]."')",
		'ico delete',
		$label,
		$label
	));
...
<thead>
  <tr>
    ...
    <th align="center" style="width:95px;">รหัส</th>
    <th align="center" >ชื่อแหล่งที่มาของเงินนอกงบประมาณ</th>
...
?>
  <tr>
    ...
    <td valign="top" ><?php echo $SourceCode;?></td>
