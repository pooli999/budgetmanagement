...
function icoEdit($r){
	...
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript:self.location='?mod=".LURL::dotPage($addPage)."&id=".$r->IndTypeId."&start=".$_REQUEST["start"]."'",
		'ico edit',
		$label,
		$label
	));
...
function icoDelete($r){
	...
	global $actionPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript: toDelete('?mod=".LURL::dotPage($actionPage)."&action=delete&id=".$r->IndTypeId."&start=".$_REQUEST["start"]."')",
		'ico delete',
		$label,
		$label
	));
...
<thead>
  <tr>
    ...
    <th align="left" >ชื่อประเภทตัวชี้วัด(KPI)</th>
