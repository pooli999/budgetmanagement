...
function icoActive($r){
	...
	$label = $r->EnableStatus == 'Y' ? 'เปิดใช้งาน' : 'ไม่ใช้งาน';
	$css = $r->EnableStatus == 'Y' ? 'enabled' : 'disabled';
	$e = $r->EnableStatus == 'Y' ? 'N' : 'Y';
	global $actionPage;
	vprintf('<a href="%s" class="%s" title="%s"><span>%s</span></a>',array(
		"javascript: toEnablestatus('?mod=".LURL::dotPage($actionPage)."&start=".$_REQUEST["start"]."&action=changestatus&id=".$r->AcChartId."&EnableStatus=".$e."&AcSeriesId=".$_REQUEST["AcSeriesId"]."')",
		'ico '.$css,
		$label,
		$label
	));
...
<thead>
  <tr>
    ...
    <th align="center" style="width:95px;">ชนิด</th>
		<th align="center" style="width:120px;">เปิดใช้งาน</th>
