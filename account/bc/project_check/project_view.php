...
if($_REQUEST['PrjId']){
	$dataPrj=$get->getProjectDetail($_REQUEST['BgtYear'], $_REQUEST['OrganizeCode'], $_REQUEST['SCTypeId'], $_REQUEST['ScreenLevel'],$_REQUEST['PrjId'],$_REQUEST['PrjDetailId']);//ltxt::print_r($dataPrj);
...
<?php if($StatusProject==2){ ?>
<input type="button" class="btnRed" name="save" id="save" value="  บันทึกผลตรวจสอบ  " onClick="window.location.href='?mod=<?php echo LURL::dotPage($addPage); ?>&SCTypeId=<?php echo $_REQUEST["SCTypeId"]; ?>&BgtYear=<?php echo $_REQUEST["BgtYear"]; ?>&OrganizeCode=<?php echo $_REQUEST["OrganizeCode"]; ?>&ScreenLevel=<?php echo $_REQUEST["ScreenLevel"]; ?>&PrjId=<?php echo $PrjId; ?>&PrjDetailId=<?php echo $PrjDetailId; ?>'" />
<?php }?>
<input type="button" class="btn" name="Cancel" id="Cancel" value="  ย้อนกลับ  " onClick="history.back(-1);" /> 
</div>