	...
	*/
	function getDataList(&$list,$limit=20){
		...
		if($_REQUEST["tsearch"]){
			...
			$where[] = "(tblbudget_finance_income.IncomeId IN(select IncomeId from  ac_action where PV like '%".$_REQUEST["tsearch"]."%'))";
			$where[] = " IncomeDetailAccount like '%".$_REQUEST["tsearch"]."%'";
		}
		...
		if($_REQUEST["tsearch"]){
			$crt1 = " and (tblbudget_finance_income.DeleteStatus='N')";
		}else{
			$crt1 = " WHERE (tblbudget_finance_income.DeleteStatus='N')";
		}

		$fn1 = '(select PV from ac_action where ac_action.IncomeId = tblbudget_finance_income.IncomeId) as PV';
		$sql="select *,$fn1 from tblbudget_finance_income ".$where_r.$crt1."  order by Ordering desc";
		//echo $sql;
...
,tblfinance_payment_list.DocCode,tblbudget_project.PrjCode from tblfinance_payment_list inner join tblfinance_doccode on tblfinance_payment_list.DocCode=tblfinance_doccode.DocCode inner join tblbudget_project on tblfinance_doccode.PrjId = tblbudget_project.PrjId ".$where_r." ";
	...
	function getaccount(){
		...
		if ($AcActionId == ""){ // undefined load ผูกบัญชี
				...
				//	echo $sql;
				}else if ($ReceiveType == 1){//ดึงบัญชีเงินสด
