...
class sFunction extends sHelper{
	...
	*/	
	function Save(){
		if($pk = $this->db->arecSave('tblbudget_finance_bookbank_ac',$_POST)){	
			if($_POST["BookbankAcId"]=='')
				LogFiles::SaveLog("ระบบบัญชี","เพิ่มข้อมูลผูกบัญชีธนาคาร",$pk,$_REQUEST["BookbankNumber"]);
			else
				LogFiles::SaveLog("ระบบบัญชี","แก้ไขข้อมูลผูกบัญชีธนาคาร",$pk,$_REQUEST["BookbankNumber"]);
