...
function ValidatePV(mmode){
    ...
    JQ.ajax({
      type: "POST",
      async: false,
      url: "?mod=<?php echo LURL::dotPage('otherjournal_action');?>",
      data: "action=chkpv&PV="+JQ("#PV").val()+"&opv="+opv,
...
function loadpv(){
	...
	mm = "<?php echo date(m);?>";
	yy = "<?php echo date(Y)+543;?>";
	if (journalId != ""){
		JQ.ajax({
			  type: "GET",
			  url: "?mod=<?php echo LURL::dotPage('otherjournal_action');?>",
			  data: "action=loadpv&journalId="+journalId+"&mm="+mm+"&yy="+yy,
			  success: function(msg){
				JQ("#PV").val(msg);
			  }
		});
...
function Save(f){
	...
	<?php if ($AcActionId==""){?>
		ans = ValidatePV("add");
	<?php }else{?>
		ans = ValidatePV("edit"); // กรณีแก้ไขไม่ต้องเช็ค
	<?php }?>
...
function loaddata(AcActionId){
	...
	JQ.ajax({
	  ...
	  //data: "action=findeaccountadd&AcActionId="+AcActionId+"&PaymentId=<?//=$_REQUEST["id"]?>",
		data: "action=findeaccountadd&AcActionId="+AcActionId,
	  success: function(result3){
			if (result3.length > 0){
				...
				for (list_d = 0;list_d < 3;list_d++){
					arr_ac_chart_id[list_d]="";
					arr_AcChartCode[list_d]="";
					arr_ThaiName[list_d]="";
					arr_textDetail[list_d]="";
					arr_DrValue[list_d]="0.00";// ค่าใช้จ่าย รายจ่าย ลง dr อย่างเดียว
					arr_CrValue[list_d]="0.00";
	  		}
...
<input id = "AcSource" name = "AcSource" type="hidden" value="1" />
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="4" class="tbl-view">
      <tr>
        ...
        <td width="30%" bgcolor="f5f2ed">
				...
				<?php// if ($AcActionId==""){?>
					...
					<input id = "opv" name = "opv" type="hidden" value = "<?= $PV?>" />
				<?php// }else{?>
      ...
      <tr>
        ...
        <td bgcolor="f5f2ed">
				<?php
						$tag_attribs = 'onchange="loadpv()" style="width:120px"';
						echo $get->getJournalNameSelect("journalId",$tag_attribs,$journalId,"เลือก");//$tag_name,$tag_attribs,$selected,$lebel
				?>
        </td>
      ...
      <tr>
        ...
        <td bgcolor="f5f2ed" colspan="5">
					<textarea id="AcDescription" name="AcDescription" cols="205" rows="4" class="input_style"><?php echo $AcDescription?></textarea>
        </td>
