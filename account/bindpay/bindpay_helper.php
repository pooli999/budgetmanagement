...
class sHelper
{
	...
	*/
	function getDataList(&$list,$limit=20){
		if($_REQUEST["AcSeriesId"]!="0"){
			$where0="tblbudget_init_cost_item.DeleteStatus='N'";
			$where1 = "tblbudget_init_cost_item_ac.AcSeriesId =".$_REQUEST["AcSeriesId"];

			$sql="select *,";
			//$sql = $sql."(select CONCAT(AcChartCode,' | ',ThaiName) from tblbudget_init_cost_item_ac inner join ac_chart on tblbudget_init_cost_item_ac.AcChartId=ac_chart.AcChartId where tblbudget_init_cost_item.CostItemId=tblbudget_init_cost_item_ac.CostItemId and ".$where1.") as acName,";
			//$sql = $sql."(select CostItemAcId from tblbudget_init_cost_item_ac where tblbudget_init_cost_item.CostItemId=tblbudget_init_cost_item_ac.CostItemId and ".$where1.") as CostItemAcId,";
			//$sql = $sql."(select AcGroupId from tblbudget_init_cost_item_ac where tblbudget_init_cost_item.CostItemId=tblbudget_init_cost_item_ac.CostItemId and ".$where1.") as AcGroupId,";
			$sql = $sql."'".$_REQUEST["AcSeriesId"]."' as AcSeriesId,'".$_REQUEST["AcSeriesName"]."' as AcSeriesName ";
			$sql = $sql."from tblbudget_init_cost_item where ".$where0."  order by tblbudget_init_cost_item.CostItemId ASC";
			//echo $sql;
	...
	/*END*/
	function getAcDataList($CostItemId){
		if($_REQUEST["AcSeriesId"]!="0"){
			$where1 = "tblbudget_init_cost_item_ac.CostItemId =".$CostItemId;
			$sql = "select CONCAT(AcChartCode,' | ',ThaiName) as acName from tblbudget_init_cost_item_ac inner join ac_chart on tblbudget_init_cost_item_ac.AcChartId=ac_chart.AcChartId where ".$where1;
			//echo $sql."aaaa";
			$this->db->setQuery($sql);
			$list = $this->db->loadDataSet();
			return $list;
	...
	*/
	function getDetail($CostItemId,$CostItemAcId){
		$where 	  = array();
		if($CostItemId){
			$where[] ="tblbudget_init_cost_item.CostItemId=".$CostItemId;
		}
		if($CostItemAcId){
			$where[] ="tblbudget_init_cost_item_ac.CostItemAcId=".$CostItemAcId;
		}
		if($where){
			$where_r = "\n where ".implode(" and ",$where);
		}

		$sql = "select * from tblbudget_init_cost_item left join tblbudget_init_cost_item_ac on tblbudget_init_cost_item.CostItemId =tblbudget_init_cost_item_ac.CostItemId ".$where_r;
		//echo $sql;
		$this->db->setQuery($sql);
...
//		$datas = array_merge($datas);
	...
	function GetAcData(){
		$where 	  = array();
		$CostItemId = ltxt::getVar( 'CostItemId','post' );
		$where[] = "tblbudget_init_cost_item_ac.CostItemId=$CostItemId";
		if($where){
			$where_r = "\n where ".implode(" and ",$where);
		}
		$data_send=array();
		$sql = "select tblbudget_init_cost_item_ac.AcChartId as AcChartId, CONCAT(AcChartCode,' | ',ThaiName) as Cname from tblbudget_init_cost_item_ac inner join ac_chart on tblbudget_init_cost_item_ac.AcChartId   = ac_chart.AcChartId  ".$where_r;
		//echo $sql;
		$this->db->setQuery($sql);
